...
def action_audit(document_path: str):
    ...
    validator_dll = SCRIPT_LOCATION / "validator" / "DocxChecker.dll"
    runtime = guarantee_dotnet() if validator_dll.exists() else None
...
def direct_paragraph_runs(paragraph: ET.Element) -> List[ET.Element]:
    """Return direct child runs only, matching comment writer semantics."""
    return [child for child in list(paragraph) if child.tag == f"{{{W_NS}}}r"]


def direct_run_text(run: ET.Element) -> str:
    """Extract concatenated text from a direct run."""
    return "".join(node.text or "" for node in run.findall(".//w:t", NSMAP))


def paragraph_direct_text(paragraph: ET.Element) -> str:
    """Return direct-run text used by the comment writer anchor model."""
    return "".join(direct_run_text(run) for run in direct_paragraph_runs(paragraph))
...
def find_ancestor_by_tag(
    node: ET.Element,
    parent_map: dict[ET.Element, ET.Element],
    local_tag: str,
) -> Optional[ET.Element]:
    """Ascend to nearest ancestor by OOXML local tag name."""
    cursor: Optional[ET.Element] = node
    target = f"{{{W_NS}}}{local_tag}"
    while cursor is not None:
...
def element_ordinal_within_parent(child: ET.Element, local_tag: str) -> Optional[int]:
    """Return 1-based ordinal for the child among siblings of the same tag."""
    parent = child.getparent() if hasattr(child, "getparent") else None
    if parent is None:
        return None
    matched = [item for item in list(parent) if item.tag == f"{{{W_NS}}}{local_tag}"]
...
def element_ordinal_from_parent_map(
    child: ET.Element,
    parent_map: dict[ET.Element, ET.Element],
    local_tag: str,
) -> Optional[int]:
    """Return 1-based ordinal using ElementTree parent map."""
...
def normalize_review_text(text: str) -> str:
    """Collapse whitespace for stable review comparisons."""
    return re.sub(r"\s+", "", text or "")


def build_direct_run_map(paragraph: ET.Element) -> List[dict[str, Any]]:
    """Return direct run spans aligned with comment writer source text."""
    ...
    for run in direct_paragraph_runs(paragraph):
        ...
        runs.append({"text": text, "start": start, "end": end})
        cursor = end
    return runs


def extract_review_map(document_path: Path) -> dict[str, Any]:
    """Extract stable paragraph/run coordinates for QA review and comment anchoring."""
    if not document_path.exists():
        raise ValueError(f"Not found: {document_path}")
    ...
    table_index_map = {id(table): index for index, table in enumerate(tables, start=1)}

    entries: List[dict[str, Any]] = []
    logical_index = 0

    for xml_index, paragraph in enumerate(paragraphs, start=1):
        text = paragraph_direct_text(paragraph)
        normalized_text = normalize_review_text(text)
        runs = build_direct_run_map(paragraph)
        if text.strip():
            logical_index += 1

        table = find_ancestor_by_tag(paragraph, parent_map, "tbl")
        row = find_ancestor_by_tag(paragraph, parent_map, "tr")
        cell = find_ancestor_by_tag(paragraph, parent_map, "tc")

        document_zone = "table" if table is not None else "body"
        table_index = table_index_map.get(id(table)) if table is not None else None
        row_index = (
            element_ordinal_from_parent_map(row, parent_map, "tr") if row is not None else None
        )
        cell_index = (
            element_ordinal_from_parent_map(cell, parent_map, "tc") if cell is not None else None
        )

        entries.append(
            {
                "paragraph_id": f"p-{xml_index:05d}",
                "xml_index": xml_index,
                "logical_index": logical_index if text.strip() else None,
    ...
    return {
        "schema_version": "minimax-docx.review-map.v1",
        "document": str(document_path),
        "generated_at": time.strftime("%Y-%m-%dT%H:%M:%S"),
        "paragraphs": entries,
        "stats": {
            "paragraph_total": len(entries),
            "text_paragraph_total": sum(1 for item in entries if item["text"].strip()),
            "table_paragraph_total": sum(1 for item in entries if item["document_zone"] == "table"),
        },
    }


def action_review_map(document_path: str, output_path: str) -> None:
    """Write review-map JSON for downstream QA tooling."""
    source = Path(document_path)
    target = Path(output_path)
    payload = extract_review_map(source)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"+ Review map written: {target}")
...
Commands:
  ...
  review-map INPUT OUTPUT  Extract OOXML-aligned review map for downstream QA/comment workflows
...
def main():
    ...
    elif command == "review-map":
        if len(sys.argv) < 4:
            print("Usage: python docx_engine.py review-map <document.docx> <output.json>")
            sys.exit(1)
        action_review_map(sys.argv[2], sys.argv[3])
    elif command == "order":
        action_order(
