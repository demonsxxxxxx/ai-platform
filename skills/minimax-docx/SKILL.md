...
## Python Role Boundary

- Python is an execution runtime, not a review responsibility boundary.
- Python in this skill is limited to deterministic parsing, patching, and validation workflows.
- Prefer automatic interpreter detection (`python` -> `py -3`) before asking user for a Python path.
- Do not request user Python path unless runtime detection fails.
- This skill does not insert new Word comments for QA findings.
