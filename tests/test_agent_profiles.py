...
async def test_agent_profile_revision_insert_types_nullable_publisher_and_preserves_parameter_order(
    status,
    published_by,
    ...
    class RecordingConnection:
        ...
        async def execute(self, sql, params):
            ...
            if "pg_advisory_xact_lock" in lowered:
                return Cursor()
            if "select coalesce(max(revision), 0) as current_revision" in lowered:
                return Cursor({"current_revision": 7})
            if "insert into agent_profile_revisions" in lowered:
                return Cursor(
                    {
                        "tenant_id": params[0],
                        "agent_id": params[1],
                        ...
                        "created_at": "database-created-at",
    ...
    conn = RecordingConnection()
    saved = await create_agent_profile_revision(
        conn,
        tenant_id="tenant-a",
        agent_id="agt_support",
        status=status,
