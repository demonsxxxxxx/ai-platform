...
def test_adapter_artifact_seam_delegates_collection_to_deliverable_runtime(monkeypatch, tmp_path):
    captured = {}
    ...
    monkeypatch.setattr(claude_agent_worker, "collect_workspace_artifacts", collect)
    adapter = ClaudeAgentWorkerAdapter(delegate=FakeDelegate())

    assert adapter._collect_workspace_artifacts(payload(file_ids=[]), tmp_path) == []
    assert captured["source_executor"] == "claude-agent-worker"
    assert captured["workspace"] == tmp_path
    assert captured["artifact_dirs"] == adapter._workspace_artifact_dirs
