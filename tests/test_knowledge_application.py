...
async def _identity_authority(_tenant_id: str, values: list[str]) -> list[str]:
    return values
...
def _service(
    *,
    ...
    return KnowledgeControlPlane(
        ...
        role_authority_validator=role_authority or _identity_authority,
        user_authority_validator=user_authority or _identity_authority,
    )
...
@pytest.mark.asyncio
async def test_source_acl_update_replaces_department_role_and_user_scopes() -> None:
    source = {
        "id": "ksrc_a",
    ...
    repository = Repository()
    audit = _Audit()
    role_calls: list[tuple[str, list[str]]] = []
    user_calls: list[tuple[str, list[str]]] = []

    async def role_authority(tenant_id: str, values: list[str]) -> list[str]:
        role_calls.append((tenant_id, values))
        return values

    async def user_authority(tenant_id: str, values: list[str]) -> list[str]:
        user_calls.append((tenant_id, values))
        return values
    ...
    await service.replace_source_acl(
        ...
        user_ids=["employee-b", "employee-b"],
    )

    assert repository.replacement["department_ids"] == ("财务部",)
    assert repository.replacement["roles"] == ("finance_reviewer",)
    assert repository.replacement["user_ids"] == ("employee-b",)
    assert role_calls == [("default", ["finance_reviewer"])]
    assert user_calls == [("default", ["employee-b"])]
    assert audit.events[0]["payload"] == {
...
@pytest.mark.parametrize(
    ("roles", "user_ids", "authority", "expected_code"),
    [
        (["unknown_role"], [], "role", "knowledge_source_acl_identity_invalid"),
        ([], ["missing-user"], "user", "knowledge_source_acl_identity_invalid"),
        (
            [],
            ["user-a"],
            "user",
...
async def test_source_acl_rejects_unknown_or_unavailable_role_and_user_authority(
    roles: list[str],
    user_ids: list[str],
    authority: str,
    expected_code: str,
    ...
    class Repository:
        async def get_source_acl_by_operation(self, _conn, **_kwargs):
            raise AssertionError("invalid authority must fail before persistence")

    async def role_authority(_tenant_id: str, values: list[str]) -> list[str]:
        if authority == "role" and values:
            raise KnowledgeError(expected_code)
        return values

    async def user_authority(_tenant_id: str, values: list[str]) -> list[str]:
        if authority == "user" and values:
            raise KnowledgeError(expected_code)
        return values
    ...
    with pytest.raises(KnowledgeError, match=expected_code):
        await service.replace_source_acl(
            tenant_id="default",
            actor_id="admin-a",
            source_id="ksrc-a",
