...
async def test_worker_loads_attempt_bound_answer_without_waiting_for_stream_publication(
    monkeypatch,
    retryable_receipt,
):
    ...
    monkeypatch.setattr(
        "app.streaming.application.worker_publication_v4.publish_run_event",
        no_terminal_publish,
    )
    monkeypatch.setattr("app.worker.repositories.mark_run_running", mark_run_running)
    monkeypatch.setattr("app.worker.repositories.append_event", append_event)
    ...
    assert calls[0] == ("adapter", queue_attempt_id)
    assert next(call[2] for call in calls if call[0] == "load") == queue_attempt_id
