...
def test_citation_markers_are_same_run_bounded_and_first_use_ordered() -> None:
    available = ("kev_first", "kev_second")

    assert ordered_citation_evidence_ids(
        "Answer [kev_second], then [kev_first], then [kev_second].",
        available,
    ) == ("kev_second", "kev_first")
    with pytest.raises(KnowledgeError, match="knowledge_citation_invalid"):
        ordered_citation_evidence_ids("Answer [kev_other].", available)
    with pytest.raises(KnowledgeError, match="knowledge_citation_invalid"):
        ordered_citation_evidence_ids("Answer without citations.", available)
...
@pytest.mark.asyncio
async def test_cancel_fence_accepts_only_the_exact_cancel_requested_successor() -> None:
    class Cursor:
    ...
    class Connection:
        async def execute(self, _query: str, _params: object) -> Cursor:
            return Cursor(
                {
                    "id": "attempt-knowledge-runtime",
    ...
    repository = PostgresKnowledgeRuntimeRepository()
    accepted = await repository._lock_run_attempt_fence(
        Connection(),
        tenant_id="tenant-knowledge-runtime",
        run_id="run-knowledge-runtime",
        attempt_id="attempt-knowledge-runtime",
    ...
    assert accepted["owner_generation"] == 5

    with pytest.raises(KnowledgeError, match="knowledge_retrieval_fence_stale"):
        await repository._lock_run_attempt_fence(
            Connection(),
            tenant_id="tenant-knowledge-runtime",
            run_id="run-knowledge-runtime",
            attempt_id="attempt-knowledge-runtime",
...
@pytest.mark.asyncio
async def test_runtime_loads_connection_revision_from_lifecycle_receipt() -> None:
    class Cursor:
        async def fetchone(self) -> dict[str, object]:
            return {
                "revision_id": "krev_policy_1",
                "connection_id": "kconn_policy",
                "revision": 1,
                "provider_key": "ragflow",
                "base_url": "https://ragflow.internal",
                "secret_ref": "secret-policy",
    ...
    class Connection:
        ...
        async def execute(self, query: str, params: object) -> Cursor:
            self.query = " ".join(query.split())
            self.params = params
            return Cursor()

    conn = Connection()
    row = await PostgresKnowledgeRuntimeRepository().load_connection_revision(
        conn,
        tenant_id="tenant-knowledge-runtime",
        connection_id="kconn_policy",
        revision_id="krev_policy_1",
        revision=1,
        catalog_sync_id="ksync_policy_1",
        lifecycle_epoch=1,
    )

    assert "join knowledge_connection_lifecycle_receipts receipts" in conn.query
    assert "knowledge_connection_activation_receipts" not in conn.query
    assert conn.params == (
        1,
        "ksync_policy_1",
        "tenant-knowledge-runtime",
        "kconn_policy",
        "krev_policy_1",
        1,
    )
    assert row is not None and row["activation_state"] == "active"


@pytest.mark.asyncio
async def test_credential_deadline_check_binds_exact_retrieval_authority() -> None:
    class Cursor:
        async def fetchone(self) -> dict[str, object]:
            return {"deadline_live": True}
    ...
    conn = Connection()
    live = await PostgresKnowledgeRuntimeRepository().retrieval_deadline_is_live(
        conn,
        tenant_id="tenant-knowledge-runtime",
        run_id="run-knowledge-runtime",
        attempt_id="attempt-knowledge-runtime",
    ...
    assert live is True
    assert "attempts.status = 'retrieving'" in conn.query
    assert "attempts.deadline_at > clock_timestamp()" in conn.query
    assert conn.params == (
        "tenant-knowledge-runtime",
        "run-knowledge-runtime",
        "attempt-knowledge-runtime",
        4,
        "a" * 64,
    )
...
async def test_runtime_repository_pins_snapshot_fences_generation_and_commits_evidence() -> (
    ...
    try:
        ...
        async with conn.transaction():
            await conn.execute(
                ...
                insert into messages(
                  id, tenant_id, session_id, run_id, role, content
                ) values (
                  'message-knowledge-cited', %s, 'session-knowledge-runtime',
                  'run-knowledge-runtime', 'assistant', 'Policy [kev_policy_1]'
                )
                """,
                (snapshot.tenant_id,),
            )
            citation_ids = await repository.resolve_citation_evidence_ids(
                conn,
                tenant_id=snapshot.tenant_id,
                run_id=snapshot.run_id,
                attempt_id="attempt-knowledge-runtime",
                answer="Policy [kev_policy_1]",
            )
            citations = await repository.finalize_citations(
                conn,
                tenant_id=snapshot.tenant_id,
                run_id=snapshot.run_id,
                attempt_id="attempt-knowledge-runtime",
                message_id="message-knowledge-cited",
                answer="Policy [kev_policy_1]",
                operation_id="knowledge-citations:run-knowledge-runtime:attempt-knowledge-runtime",
            )
        assert citation_ids == ("kev_policy_1",)
        assert [item["evidence_id"] for item in citations] == ["kev_policy_1"]
        assert citations[0]["title"] == "Policy"
        assert citations[0]["position"] == {"page": 3}

        async with conn.transaction():
            replayed_citations = await repository.finalize_citations(
                conn,
                tenant_id=snapshot.tenant_id,
                run_id=snapshot.run_id,
                attempt_id="attempt-knowledge-runtime",
        ...
        assert replayed_citations == citations
            ...
            async with conn.transaction():
                await conn.execute(
                    ...
                    insert into messages(
                      ...
                      'message-knowledge-invalid', %s, 'session-knowledge-runtime',
                      'run-knowledge-runtime', 'assistant', 'Invalid [kev_other]'
                    )
                    """,
                    (snapshot.tenant_id,),
                )
                await repository.finalize_citations(
                    conn,
                    tenant_id=snapshot.tenant_id,
                    run_id=snapshot.run_id,
                    attempt_id="attempt-knowledge-runtime",
                    message_id="message-knowledge-invalid",
                    answer="Invalid [kev_other]",
                    operation_id="knowledge-citations:invalid",
                )
        missing_message = await conn.execute(
            "select 1 from messages where id = 'message-knowledge-invalid'"
        )
        assert await missing_message.fetchone() is None

        with pytest.raises(psycopg.errors.CheckViolation):
            async with conn.transaction():
                await conn.execute(
                    """
                    update knowledge_citations set title = 'mutated'
                    where tenant_id = %s and run_id = %s and message_id = %s
                    """,
                    (
                        snapshot.tenant_id,
                        snapshot.run_id,
                        "message-knowledge-cited",
                    ),
                )
