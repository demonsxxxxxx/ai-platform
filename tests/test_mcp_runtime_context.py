...
@pytest.mark.asyncio
async def test_gateway_revision_reader_streams_and_stops_at_response_limit(monkeypatch):
    yielded = []
    ...
    class Response:
        ...
        async def aiter_raw(self):
            for chunk in (b'{"catalog_revision":1,', b"x" * 16_384, b'"acl_revision":1}'):
    ...
    class Client:
        ...
        def stream(self, method, url, **kwargs):
            assert method == "GET"
            assert url == "https://127.0.0.1/api/internal/cache-revisions"
            assert kwargs["headers"]["X-MCP-Gateway-Service-Token"] == "service-token"
            assert kwargs["headers"]["Accept-Encoding"] == "identity"
            return Response()

        async def get(self, *_args, **_kwargs):
            raise AssertionError("revision reader must not buffer through client.get")

    async def validate(_endpoint):
        return McpValidatedTarget(
            endpoint="https://gateway.example/api/internal/cache-revisions",
            connect_url="https://127.0.0.1/api/internal/cache-revisions",
            host_header="gateway.example",
            sni_hostname="gateway.example",
        )

    monkeypatch.setattr(mcp_runtime, "validate_registered_mcp_target", validate)
    monkeypatch.setattr(mcp_runtime.httpx, "AsyncClient", lambda **_kwargs: Client())

    assert await mcp_runtime.read_gateway_cache_revisions(
...
@pytest.mark.asyncio
async def test_gateway_revision_reader_accepts_bounded_streamed_json(monkeypatch):
    class Response:
        status_code = 200
        headers = {}
        ...
        async def aiter_raw(self):
            yield b'{"catalog_revision":7,'
            yield b'"acl_revision":11}'
    ...
    assert await mcp_runtime.read_gateway_cache_revisions(
        "https://gateway.example/mcp",
        service_token="service-token",
    ) == {"catalog_revision": 7, "acl_revision": 11}


@pytest.mark.asyncio
async def test_gateway_revision_reader_rejects_compressed_response_before_body(monkeypatch):
    class Response:
        status_code = 200
        ...
        async def aiter_raw(self):
            raise AssertionError("compressed revision body must not be read")
            yield b""
