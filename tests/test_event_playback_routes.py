...
def test_ordinary_activity_routes_use_fixed_envelopes_for_syntax_safe_executor_values(monkeypatch):
    ...
    activity_events = [
        ...
        event_row(
            event_id="evt-preparing",
            sequence=5,
            event_type="intent_detected",
            stage=raw_terms[2],
            payload={"prompt": raw_terms[0], "runtime_path": raw_terms[3]},
            message=raw_terms[2],
        ),
        event_row(
            event_id="evt-skill-ready",
            sequence=6,
            event_type="skill_selected",
            stage=raw_terms[2],
            payload={"skill_id": raw_terms[0], "private_payload": {"value": raw_terms[3]}},
            message=raw_terms[2],
        ),
        event_row(
            ...
            stage=raw_terms[2],
            payload={"heartbeat": False, "runtime_path": raw_terms[3]},
            message=raw_terms[2],
        ),
        event_row(
            ...
            event_type="run_started",
            stage=raw_terms[2],
            payload={"heartbeat": True, "source": raw_terms[3]},
            message=raw_terms[2],
        ),
        event_row(
    ...
    assert [(event["event_type"], event["stage"], event["message"], event["payload"]) for event in events] == [
        ...
        ("intent_detected", "preparation", "正在准备受控运行请求。", {"activity": {"category": "preparation", "status": "running"}}),
        ("capability_selected", "capability", "已加载授权处理能力。", {"activity": {"category": "capability", "status": "completed"}}),
        ("run_started", "execution", "已完成请求准备，正在执行任务。", {"activity": {"category": "execution", "status": "running"}}),
        ("heartbeat", "liveness", "任务仍在运行。", {"activity": {"category": "liveness", "status": "running", "meaningful": False}}),
