...
async def test_stage_stale_cancel_requested_run_uses_scoped_cas_and_existing_cancel_contract(monkeypatch):
                ...
                return SingleRowCursor(
                    {
                        "id": "run-a",
                        "trace_id": "trace-a",
                        "terminalization_target": "cancelled",
                    }
                )
    ...
    assert row == {"id": "run-a", "trace_id": "trace-a", "terminalization_target": "cancelled"}
...
async def test_stage_stale_running_run_fails_explicitly_and_cas_loss_emits_nothing(monkeypatch):
    ...
    failed = await repositories.stage_stale_run_reconciliation(
        Connection(
            {"id": "run-failed", "trace_id": "trace-failed", "terminalization_target": "failed"}
        ),
        tenant_id="tenant-a",
        workspace_id="workspace-a",
        user_id="user-a",
...
async def test_cancel_run_closes_non_terminal_run_steps(monkeypatch):
    class RecordingConnection:
        ...
        async def execute(self, sql, params):
            normalized = " ".join(sql.split())
            self.calls.append((normalized, params))
            if "set terminalization_target" in normalized:
                return SingleRowCursor(
                    {
                        "id": "run-a",
                        "trace_id": "trace-a",
            ...
            if normalized.startswith("select id, trace_id, status, terminalization_target"):
                return SingleRowCursor(
                    {
                        "id": "run-a",
                        "terminalization_target": "cancelled",
                        "terminalization_reason": "run_cancelled",
                    }
                )
    ...
    assert "set terminalization_target" in conn.calls[0][0]
    assert conn.calls[0][1][0:4] == ("cancelled", "cancelled", "cancelled", "run_cancelled")
    assert conn.calls[0][1][-2:] == ("tenant-a", "run-a")
    assert conn.calls[1][0].startswith("select id, trace_id, status, terminalization_target")
    assert "set status = 'cancelled'" in conn.calls[2][0]
    assert all("run_tool_permission_requests" not in sql for sql, _params in conn.calls)
...
async def test_fail_run_closes_non_terminal_run_steps_without_leaving_stale_progress(monkeypatch):
                ...
                return SingleRowCursor(
                    {
                        ...
                        "terminalization_result_json": {"message": "failed"},
                        "terminalization_error_code": "executor_failure",
                        "terminalization_error_message": "boom",
                    }
                )
            if "has_unterminalized" in normalized:
                return SingleRowCursor({"has_unterminalized": False})
    ...
    assert conn.calls[2][0].startswith("select id, trace_id, status, terminalization_target")
    assert "set status = 'failed'" in conn.calls[3][0]
    assert all("run_tool_permission_requests" not in sql for sql, _params in conn.calls)
    step_updates = [call for call in conn.calls if call[0].startswith("update run_steps")]
    assert len(step_updates) == 1
...
async def test_terminal_run_writes_do_not_overwrite_existing_terminal_status():
    class TerminalRecordingConnection(RecordingConnection):
        async def execute(self, sql, params):
            normalized = " ".join(sql.split())
            if "set terminalization_target = case" in normalized:
                self.calls.append((normalized, params))
                return SingleRowCursor({"terminalization_target": params[0]})
            return await super().execute(sql, params)
    ...
    assert "terminalization_target is null" in completion_lock_sql
    assert "where runs.tenant_id = %s and runs.id = %s" in completion_sql

    staged_terminal_updates = [sql for sql in update_runs_sql if "terminalization_target = case" in sql]
