...
from app.bootstrap import knowledge as knowledge_bootstrap
...
@pytest.mark.asyncio
async def test_knowledge_acl_composition_validates_role_catalog_and_active_users(
    monkeypatch,
) -> None:
    assert await knowledge_bootstrap._validate_knowledge_role_authorities(
        "default", ["auditor"]
    ) == ["auditor"]
    with pytest.raises(KnowledgeError, match="knowledge_source_acl_identity_invalid"):
        await knowledge_bootstrap._validate_knowledge_role_authorities(
    ...
    async def active_users(_conn, **_kwargs):
        return ("user-a",)

    monkeypatch.setattr(knowledge_bootstrap, "transaction", transaction)
    monkeypatch.setattr(knowledge_bootstrap, "list_active_user_ids", active_users)
    assert await knowledge_bootstrap._validate_knowledge_user_authorities(
        "default", ["user-a"]
    ) == ["user-a"]
    with pytest.raises(KnowledgeError, match="knowledge_source_acl_identity_invalid"):
        await knowledge_bootstrap._validate_knowledge_user_authorities(
            "default", ["missing-user"]
        )

    async def unavailable(_conn, **_kwargs):
        raise RuntimeError("private identity outage")

    monkeypatch.setattr(knowledge_bootstrap, "list_active_user_ids", unavailable)
    with pytest.raises(
        KnowledgeError,
        match="knowledge_source_acl_identity_authority_unavailable",
    ) as caught:
        await knowledge_bootstrap._validate_knowledge_user_authorities(
            "default", ["user-a"]
        )
    assert "private identity outage" not in str(caught.value)
...
@pytest.mark.asyncio
async def test_ragflow_provider_maps_tls_failure_to_safe_connection_error(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        socket,
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        try:
            ...
            raise httpx.ConnectError("private transport detail", request=request) from exc
    ...
    with pytest.raises(KnowledgeError, match="knowledge_connection_invalid") as caught:
...
class _TransportService:
    def __init__(self) -> None:
        ...
        self.source_acl_requests: list[dict[str, object]] = []
    ...
    async def replace_source_acl(self, **kwargs):
        self.source_acl_requests.append(kwargs)
        return {"id": kwargs["source_id"], "authorization_version": 2}
...
def test_knowledge_transport_is_admin_only_and_never_echoes_credentials(monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        source_acl = client.put(
            "/api/ai/admin/knowledge/sources/ksrc_a/acl",
            json={
                "operation_id": str(uuid4()),
    ...
    assert source_acl.status_code == 200
    assert service.source_acl_requests[0]["department_ids"] == ["财务部"]
    assert service.source_acl_requests[0]["roles"] == ["finance_reviewer"]
    assert service.source_acl_requests[0]["user_ids"] == ["employee-a"]
