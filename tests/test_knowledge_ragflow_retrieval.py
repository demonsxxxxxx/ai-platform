...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ConnectionResetError("private reset detail"), "knowledge_provider_transient"),
    ],
)
async def test_ragflow_retrieval_classifies_tls_without_retrying_other_network_errors(
    monkeypatch,
    cause,
    expected_code,
) -> None:
    ...
    with pytest.raises(KnowledgeError, match=expected_code) as caught:
        await provider.retrieve(
            base_url="https://ragflow.internal:9380",
            credential="provider-key",
            request=_request(),
            control=ProviderCallControl(timeout_seconds=2),
        )
