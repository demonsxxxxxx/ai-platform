...
def test_schema_declares_external_knowledge_authority_without_plaintext_credentials():
    ...
    assert "'knowledge_query_invalid'" in knowledge
    assert "ai_platform_guard_knowledge_connection_lifecycle_receipt_immutable" in knowledge
    assert "trg_knowledge_connection_lifecycle_receipt_immutable" in knowledge
    assert "before update or delete on knowledge_connection_lifecycle_receipts" in knowledge
    assert "create table if not exists knowledge_citations" in knowledge
    assert "fk_knowledge_citation_message" in knowledge
    assert "uq_knowledge_citation_message_ordinal" in knowledge
    assert "before update or delete on knowledge_citations" in knowledge
    assert "create index if not exists idx_knowledge_citations_message" in knowledge
