...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("claude_agent_sdk_selected_skill_not_invoked", True, "claude_agent_sdk_runtime_error"),
        ("claude_agent_sdk_selected_skill_hook_failed", True, "claude_agent_sdk_runtime_error"),
...
def test_executor_execute_canonicalizes_sdk_failures_without_rewriting_specific_codes(
    ...
    assert body["error_message"] == expected_messages.get(
        expected_error_code,
        "Claude Agent SDK execution failed",
    )
    assert re.fullmatch(r"diag_[0-9a-f]{16}", body["diagnostic_id"])
    if sdk_error not in {body["error_code"], body["error_message"]}:
        assert sdk_error not in response.text
...
@pytest.mark.asyncio
async def test_executor_routes_uploaded_skill_id_to_sdk_native(monkeypatch, tmp_path):
    captured = {}
...
def test_executor_execute_does_not_rewrite_runner_timeout_error_as_deadline(tmp_path):
    ...
    assert response.json()["error_message"] == "Executor runner failed"
    assert re.fullmatch(r"diag_[0-9a-f]{16}", response.json()["diagnostic_id"])
    assert "runner dependency timed out" not in response.text
...
def test_executor_execute_classifies_decorated_runner_timeout_as_internal_failure(tmp_path):
    ...
    assert response.json()["error_message"] == "Executor runner failed"
    assert re.fullmatch(r"diag_[0-9a-f]{16}", response.json()["diagnostic_id"])
    assert "decorated runner dependency timed out" not in response.text
    assert "requested_max_seconds" not in response.json()
    assert "timeout_elapsed_ms" not in response.json()
