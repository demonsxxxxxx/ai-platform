...
async def test_stream_admission_freezes_open_envelope_before_confirmation():
    ...
    confirmed_row = authority_row(state="confirmed", open_payload_digest=insert_params[9])
...
async def test_stream_admission_rejects_a_different_attempt_instead_of_creating_parallel_authority():
    ...
    with pytest.raises(control.SseAuthorityConflictError, match="sse_stream_attempt_conflict"):
...
def test_authority_lease_checks_local_epoch_invalidation_and_deadline_without_database_io():
    ...
    assert lease.allows_frame(now=now, local_authorization_epoch=4, invalidated_through_epoch=3)
    assert not lease.allows_frame(now=now, local_authorization_epoch=5, invalidated_through_epoch=3)
    assert not lease.allows_frame(now=now, local_authorization_epoch=4, invalidated_through_epoch=4)
...
async def test_authority_lease_is_bounded_to_fifteen_seconds_and_revocation_fences_renewal():
    ...
    revoked_conn = ScriptedConnection([authority_row(state="confirmed", revocation_state="committed")])
    with pytest.raises(control.SseAuthorityConflictError, match="sse_authority_revoked"):
        await control.acquire_sse_authority_lease(
            revoked_conn,
            tenant_id="tenant-a",
            run_id="run-a",
...
def test_terminal_intent_freezes_stable_semantic_ids_exact_payload_bytes_and_hashes():
    ...
    assert control._sha256(first.terminal_payload_bytes) == first.terminal_payload_digest
...
async def test_terminal_intent_duplicate_with_different_payload_hash_fails_closed():
    ...
    with pytest.raises(control.SseAuthorityConflictError, match="sse_terminal_intent_conflict"):
