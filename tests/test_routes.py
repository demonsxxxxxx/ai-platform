...
def test_run_event_stream_omits_retired_multi_agent_step_snapshot(monkeypatch):
    from fastapi.testclient import TestClient
    ...
    async def fake_list_run_steps(conn, *, tenant_id, run_id):
        raise AssertionError("SSE must not load run steps for a retired platform snapshot")
    ...
    assert response.status_code == 200
    assert "event: multi_agent_snapshot" not in response.text
