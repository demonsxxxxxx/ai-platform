...
class _RouteCancellationHandlers:
    owner = None
    admin = None


route_cancellation_handlers = _RouteCancellationHandlers()


class _RouteCancellationAdapter:
    async def request_owner_cancel(self, *, tenant_id, owner_user_id, run_id):
        handler = route_cancellation_handlers.owner
    ...
    async def request_admin_cancel(self, *, tenant_id, admin_user_id, run_id):
        handler = route_cancellation_handlers.admin
        value = await handler(None, tenant_id=tenant_id, admin_user_id=admin_user_id, run_id=run_id)
        return _RouteCancellationReceipt(value) if value is not None else None


async def _no_terminal_progress(*_args, **_kwargs):
    return None
...
class _CancellationTestConnection:
    ...
    async def execute(self, sql, params):
        ...
        if normalized.startswith("update runs set terminalization_target"):
            return _RunAttemptCursor()
...
def _install_route_cancellation_adapter(monkeypatch):
    ...
    monkeypatch.setattr(route_cancellation_handlers, "owner", missing_cancel)
    monkeypatch.setattr(route_cancellation_handlers, "admin", missing_cancel)
...
def _install_test_run_stream_runtime(monkeypatch):
    ...
    def create_app_with_runtime():
        ...
        app.state.run_lifecycle = object()
        return app
...
def allow_existing_run_control_route_tests_to_stub_auth_snapshot_update(monkeypatch):
    ...
    monkeypatch.setattr(replay_persistence, "authorize_replay_run_capabilities", authorize_replay_capabilities)
    monkeypatch.setattr(replay_persistence, "insert_run_skill_snapshots_at_creation", insert_creation_snapshots)
...
def sandbox_lease_row(
    *,
    ...
    return {
        ...
        "runtime_container_id": runtime_container_id or f"exec-{run_id}",
        "runtime_container_name": runtime_container_name or f"executor-exec-{run_id}",
        "runtime_executor_url": runtime_executor_url or "http://executor.test",
        "runtime_workspace_container_path": runtime_workspace_container_path or "/workspace",
...
def test_copy_run_creates_new_queued_run(monkeypatch, enqueue_mode):
    ...
    execution_snapshot_calls = [item[1] for item in calls if item[0] == "execution_snapshot"]
    assert execution_snapshot_calls == [
        {
            "tenant_id": "default",
            "run_id": "run_new",
...
async def test_copy_run_as_new_task_returns_full_execution_input_for_queue(monkeypatch):
    ...
    assert copied["input"]["message"] == "build feature"
...
async def test_resume_run_as_new_task_rejects_when_resume_is_already_active(monkeypatch):
    ...
    monkeypatch.setattr(replay_persistence, "get_authorized_run", fake_get_authorized_run)
    monkeypatch.setattr(replay_persistence, "_completed_steps_for_resume", fake_completed_steps_for_resume)
...
async def test_resume_run_as_new_task_records_resume_events_and_audit(monkeypatch):
    ...
    monkeypatch.setattr(replay_persistence, "get_authorized_run", fake_get_authorized_run)
    monkeypatch.setattr(replay_persistence, "_completed_steps_for_resume", fake_completed_steps_for_resume)
    monkeypatch.setattr(replay_persistence, "get_active_resume_for_source_run", fake_get_active_resume_for_source_run)
    monkeypatch.setattr(replay_persistence, "copy_run_as_new_task", fake_copy_run_as_new_task)
...
async def test_retry_run_as_new_task_auth_snapshot_records_retry_events_and_audit(monkeypatch):
    ...
    monkeypatch.setattr(replay_persistence, "get_authorized_run", fake_get_authorized_run)
    monkeypatch.setattr(replay_persistence, "copy_run_as_new_task", fake_copy_run_as_new_task)
    monkeypatch.setattr(replay_persistence, "get_active_retry_for_source_run", fake_get_active_retry_for_source_run, raising=False)
    monkeypatch.setattr(replay_persistence, "append_event", fake_append_event)
    monkeypatch.setattr(replay_persistence, "append_audit_log", fake_append_audit_log)
...
def test_cancel_run_records_platform_cancel_request(monkeypatch):
    ...
    monkeypatch.setattr("app.auth.get_settings", auth_settings)
    monkeypatch.setattr("app.routes.runs.transaction", fake_transaction)
    monkeypatch.setattr(route_cancellation_handlers, "owner", fake_request_run_cancel)
    client = TestClient(create_app())
...
def test_cancel_run_uses_platform_verified_runtime_handle_not_user_payload(monkeypatch):
    calls = []
        ...
        return {
            ...
            "active_sandbox_leases": [
                sandbox_lease_row(
                    run_id=run_id,
                    user_id=user_id,
                    runtime_container_id="platform-runtime-container",
                    runtime_container_name="platform-runtime-name",
                    runtime_executor_url="http://platform-runtime.test",
                    runtime_workspace_container_path="/verified-workspace",
                    lease_payload_json={
                        "container_id": "ai-platform-api",
                        "container_name": "ai-platform-api",
                        "executor_url": "http://attacker.invalid",
                        "workspace_container_path": "/attacker",
                        "labels": {"controlled": "by-request"},
                    },
                )
            ],
        }
    ...
    assert response.status_code == 200
    _, lease, _ = calls[0]
    assert lease.container_id == "platform-runtime-container"
    assert lease.container_name == "platform-runtime-name"
    assert lease.executor_url == "http://platform-runtime.test"
    assert lease.workspace_container_path == "/verified-workspace"
    assert lease.labels == {}


def test_cancel_run_rejects_active_lease_without_platform_verified_runtime_handle(monkeypatch):
    calls = []
    release_calls = []
    cleanup_outcomes = []

    forged_row = sandbox_lease_row(
        run_id="run_active",
        user_id="user-a",
        lease_payload_json={
            "container_id": "ai-platform-api",
    ...
    for key in (
        ...
        forged_row.pop(key, None)

    async def fake_request_run_cancel(conn, *, tenant_id, user_id, run_id):
        return {"run_id": run_id, "status": "cancel_requested", "active_sandbox_leases": [forged_row]}
...
def test_admin_cancel_run_stops_active_sandbox_runtime_before_db_release(monkeypatch):
    ...
    monkeypatch.setattr(route_cancellation_handlers, "admin", fake_request_admin_run_cancel)
    monkeypatch.setattr(
