...
from app.execution.api import (
    PinnedSkillMismatch,
    allowed_skill_names as _allowed_skill_names,
    collect_workspace_artifacts,
    completed_step_checkpoint_payload as _completed_step_checkpoint_payload_projection,
    file_skill_steps as _file_skill_steps,
    is_file_skill_execution_step as _is_file_skill_execution_step,
    non_execution_step_output as _non_execution_step_output,
    pinned_skill_manifests as _pinned_skill_manifests,
    resume_checkpoint_lineage as _resume_checkpoint_lineage_projection,
    resume_completed_step_checkpoints as _resume_completed_step_checkpoints,
    resume_completed_step_outputs as _resume_completed_step_outputs,
    resume_copied_from_run_id as _resume_copied_from_run_id,
...
class ClaudeAgentWorkerAdapter:
    ...
    def _wrap_file_skill_result(
        self, payload: RunPayload,
        result: ExecutorResult,
        *,
        multi_agent: bool = False,
    ) -> ExecutorResult:
        public_result = {key: value for key, value in result.result.items() if key != "sdk_session_id"}
        private_payload = {
            key: value for key, value in result.executor_payload.items() if key != "sdk_session_id"
        }
        return ExecutorResult(
    ...
    async def _run_multi_agent_file_skill(
        ...
        if completed_outputs:
            ...
            if resume_preflight_failure is not None:
                return self._wrap_file_skill_result(payload, resume_preflight_failure, multi_agent=True)
        ...
        for index, step in enumerate(steps, start=1):
            ...
            if not skill_executed and _is_file_skill_execution_step(step):
                ...
                if skill_result.status != "succeeded":
                    ...
                    return self._wrap_file_skill_result(payload, skill_result, multi_agent=True)
...
def _resume_checkpoint_lineage(
    ...
    *,
    ...
    return _resume_checkpoint_lineage_projection(
        completed_checkpoints,
        step_key=step_key,
        copied_from_run_id=copied_from_run_id,
...
def _completed_step_checkpoint_payload(
    payload: RunPayload,
    *,
    step_index: int,
) -> dict[str, str]:
    return _completed_step_checkpoint_payload_projection(
        payload,
        step_index=step_index,
        lineage_contract=artifact_lineage_contract,
    )
