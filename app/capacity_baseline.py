...
from app.platform.capacity_rendering import (
    render_capacity_baseline_markdown as _render_capacity_baseline_markdown,
)
...
def render_capacity_baseline_markdown(baseline: dict[str, Any]) -> str:
    """Compatibility entry point for the extracted rendering module."""
    return _render_capacity_baseline_markdown(baseline)
...
def _admin_runtime_section_evidence(overview: dict[str, Any]) -> dict[str, object]:
    ...
    sandbox = _dict(overview.get("sandbox"))
    if _sandbox_container_observation_degraded(sandbox) and "sandbox" not in missing_sections:
...
def _sandbox_container_observation_degraded(sandbox: dict[str, Any]) -> bool:
    if sandbox.get("container_observation_degraded") is True:
        return True
...
def _snapshot_admin_runtime_evidence(snapshot: dict[str, Any]) -> dict[str, object]:
    ...
    if _sandbox_container_observation_degraded(sandbox_signals):
