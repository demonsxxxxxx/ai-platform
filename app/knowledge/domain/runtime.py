...
_CITATION_MARKER_PATTERN = re.compile(r"\[([A-Za-z0-9][A-Za-z0-9_.:-]{0,159})\]")
_MAX_SOURCE_SNAPSHOT_BYTES = 16_384
_MAX_POSITION_BYTES = 8_192
_MAX_ENGINE_EVIDENCE_BYTES = 131_072
_ENGINE_EVIDENCE_KEYS = frozenset({"evidence_id", "title", "content", "rank", "score"})
...
class KnowledgeEvidence:
    ...
    def engine_projection(self) -> dict[str, Any]:
        """Return the only evidence fields permitted to cross into an Engine."""
...
def ordered_citation_evidence_ids(
    answer: Any,
    available_evidence_ids: Any,
) -> tuple[str, ...]:
    """Extract the first-use order of valid platform evidence markers."""

    if not isinstance(answer, str) or not isinstance(
        available_evidence_ids, (list, tuple)
    ):
        raise KnowledgeError("knowledge_citation_invalid")
    try:
        available = tuple(
            _bounded_identifier(value, max_bytes=160)
            for value in available_evidence_ids
        )
    except KnowledgeError as exc:
        raise KnowledgeError("knowledge_citation_invalid") from exc
    if not 1 <= len(available) <= 20 or len(available) != len(set(available)):
        raise KnowledgeError("knowledge_citation_invalid")
    ...
    for match in _CITATION_MARKER_PATTERN.finditer(answer):
        ...
        if not evidence_id.startswith("kev_"):
            continue
...
def canonical_engine_evidence(value: Any) -> tuple[dict[str, Any], ...]:
    """Validate the complete, provider-private-free Engine evidence contract."""
    ...
    used_bytes = 0
    for expected_rank, item in enumerate(value, start=1):
        if not isinstance(item, dict) or set(item) != _ENGINE_EVIDENCE_KEYS:
            raise KnowledgeError("knowledge_evidence_invalid")
        ...
        try:
            normalized_evidence_id = _bounded_identifier(
                evidence_id,
                max_bytes=160,
            )
        except KnowledgeError as exc:
            raise KnowledgeError("knowledge_evidence_invalid") from exc
        if (
            normalized_evidence_id != evidence_id
            or normalized_evidence_id in evidence_ids
            or not isinstance(title, str)
            or len(title.encode("utf-8")) > 512
            or any(ord(character) == 0 for character in title)
            ...
            raise KnowledgeError("knowledge_evidence_invalid")
        used_bytes += len(title.encode("utf-8")) + len(content.encode("utf-8"))
        if used_bytes > _MAX_ENGINE_EVIDENCE_BYTES:
            raise KnowledgeError("knowledge_evidence_invalid")
        evidence_ids.add(normalized_evidence_id)
        projected.append(
            {
