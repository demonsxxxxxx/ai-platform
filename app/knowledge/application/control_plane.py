...
class KnowledgeControlPlane:
    def __init__(
        ...
        *,
        ...
        | None = None,
        role_authority_validator: Callable[
            [str, list[str]], Awaitable[list[str]]
        ]
        | None = None,
        user_authority_validator: Callable[
            [str, list[str]], Awaitable[list[str]]
        ]
        | None = None,
    ) -> None:
        ...
        self._role_authority_validator = role_authority_validator
        self._user_authority_validator = user_authority_validator
    ...
    async def replace_source_acl(
        ...
        *,
        ...
        if visibility == "enterprise" and any(normalized):
        ...
        if normalized[1]:
            if self._role_authority_validator is None:
                raise KnowledgeError("knowledge_source_acl_identity_authority_unavailable")
            normalized[1] = tuple(
                await self._role_authority_validator(tenant_id, list(normalized[1]))
            )
        if normalized[2]:
            if self._user_authority_validator is None:
                raise KnowledgeError("knowledge_source_acl_identity_authority_unavailable")
            normalized[2] = tuple(
                await self._user_authority_validator(tenant_id, list(normalized[2]))
            )
            ...
            if result is not None:
                await self._audit_writer.append(
                    ...
                    payload={
                        ...
                        "role_count": len(normalized[1]),
                        "user_count": len(normalized[2]),
                        "visibility": visibility,
                    },
                )
            return result
