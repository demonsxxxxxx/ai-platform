...
class ContextRetrievalInputError(ValueError):
    pass


@dataclass(frozen=True)
class ContextRetrievalIdentity:
    tenant_id: str
    workspace_id: str
    user_id: str
    session_id: str
...
_CONTEXT_ACTION_ARGUMENTS = {
    ...
    "search_memory": frozenset({"query", "limit", "max_tokens"}),
}
...
class ContextRetrievalAuthority:
    ...
    async def execute(
        self,
        action: str,
        scoped_identity: ContextRetrievalIdentity | Mapping[str, Any],
        arguments: Mapping[str, Any] | None,
    ) -> dict[str, Any]:
        ...
        if action_name == "read_session_messages":
            return await self.read_session_messages(
        ...
        if action_name == "read_run_artifact":
            return await self.read_run_artifact(
                **self._run_identity(identity),
                artifact_id=self._required_identifier(values, "artifact_id"),
                max_bytes=_bounded_int(values.get("max_bytes"), default=65536, minimum=1, maximum=262144),
            )
        if action_name == "search_memory":
            return await self.search_memory(
                ...
                limit=_bounded_int(values.get("limit"), default=10, minimum=1, maximum=50),
                max_tokens=_bounded_int(values.get("max_tokens"), default=1200, minimum=1, maximum=8000),
            )

        identifier_key = (
            "file_id" if action_name == "stage_context_file_to_workspace" else "artifact_id"
        )
        identifier = self._required_identifier(values, identifier_key)
        max_bytes = _bounded_int(
            values.get("max_bytes"),
            default=(1048576 if identifier_key == "file_id" else 16777216),
            minimum=1,
        ...
        if self._workspace_root is not None:
            stage = (
                ...
                else self.stage_run_artifact_to_workspace
            )
            return await stage(
                **self._run_identity(identity),
        ...
        export = (
            ...
            if identifier_key == "file_id"
            else self.export_run_artifact_for_broker
        )
    ...
    def _run_identity(self, identity: Mapping[str, str]) -> dict[str, str]:
        return {
            name: identity[name]
            for name in ("tenant_id", "workspace_id", "user_id", "session_id", "run_id")
        }
    ...
    def _required_identifier(arguments: Mapping[str, Any], name: str) -> str:
        ...
        if not value:
            raise ContextRetrievalInputError(f"{name}_required")
        return value
...
# Transitional name retained for internal callers while the package seam settles.
ContextRetrieval = ContextRetrievalAuthority
