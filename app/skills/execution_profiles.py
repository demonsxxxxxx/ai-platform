...
_SANDBOX_LOCAL_TOOL_IDENTITIES = (
    "Read",
    "Glob",
    "LS",
    "Bash",
...
def _native_execution_profile(*, trust_basis: str) -> SkillExecutionProfile:
    identities = _known_tool_identities(_SANDBOX_LOCAL_TOOL_IDENTITIES)
    return {
...
def resolve_skill_execution_profile(
    *,
    ...
    """Resolve the platform-owned sandbox tool profile for one immutable Skill."""
    ...
    if (
        ...
        return _native_execution_profile(trust_basis="repository_builtin")
    if source_kind == "uploaded" and normalized_status in _TRUSTED_UPLOADED_STATUSES:
        return _native_execution_profile(trust_basis="admin_reviewed_release")
    return {
        "schema_version": SKILL_EXECUTION_PROFILE_SCHEMA_VERSION,
        "strategy": SDK_RESTRICTED,
        "trust_basis": "unreviewed",
