...
projection terms. The current production scan has no forbidden or quarantined
source violations, while remaining route-policy gaps stay visible for G6/G9
follow-up. The audit also emits an active-browser route
...
- Auth/session: `/api/ai/auth/bootstrap`, `/api/ai/auth/login`,
  `/api/ai/auth/me`, `/api/ai/auth/logout`.
- Chat/session stream: `/api/chat/stream`, `/api/chat/sessions/{id}/stream`,
  `/api/sessions/*`.
- Uploads: `/api/ai/files` and `/api/ai/files/uploads/*`.
