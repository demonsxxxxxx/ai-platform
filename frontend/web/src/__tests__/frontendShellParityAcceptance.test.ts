...
test("skills hub lets PR177 public catalogs prove permissions before fail-closed", () => {
  ...
  assert.match(skillsHub, /hasPermission:\s*hubGovernance\.hasPermission/);
  ...
  assert.match(resolver, /governedUnavailable: Boolean\(catalogPermissionDenied\)/);
  ...
  assert.match(resolver, /hasPermission: !catalogPermissionDenied/);
