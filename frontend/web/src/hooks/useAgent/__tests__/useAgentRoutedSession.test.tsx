...
test("useAgent continues an ordinary failed file-Skill session with only its persisted user turn", async () => {
  const harness = await loadReactHarness();
  const { sessionApi } = await import("../../../services/api/session.ts");
  const originalGet = sessionApi.get;
  ...
  sessionApi.getEvents = async (_sessionId, options) => {
    ...
    return {
      current_run_id: "run-file-skill-failed",
  ...
  try {
    await harness.act(async () => {
      const restored = await harness.hook.loadHistory("session-file-skill-failed");
...
test("useAgent fails closed when either required session projection read fails", async () => {
  const cases = [
    { name: "compatibility session", failingRead: "session" as const },
    { name: "authoritative identity", failingRead: "authoritative" as const },
  ];

  for (const { name, failingRead } of cases) {
    const harness = await loadReactHarness();
    const { sessionApi } = await import("../../../services/api/session.ts");
    const originalGet = sessionApi.get;
    const originalGetAuthoritative = sessionApi.getAuthoritative;
    ...
    try {
      ...
      await settle(harness.act);
      assert.equal(harness.hook.error, "加载会话失败", name);
      ...
      assert.equal(submissions, 0, name);
...
test("useAgent retains verified generic authority after a retriable event history failure", async () => {
  const harness = await loadReactHarness();
  const { sessionApi } = await import("../../../services/api/session.ts");
  const originalGet = sessionApi.get;
  const originalGetAuthoritative = sessionApi.getAuthoritative;
...
test("useAgent records a bounded optional feedback failure without revoking session authority", async () => {
  const harness = await loadReactHarness({
...
test("useAgent rejects an authoritative identity mismatch without submitting", async () => {
  const harness = await loadReactHarness();
  const { sessionApi } = await import("../../../services/api/session.ts");
  const originalGet = sessionApi.get;
  const originalGetAuthoritative = sessionApi.getAuthoritative;
