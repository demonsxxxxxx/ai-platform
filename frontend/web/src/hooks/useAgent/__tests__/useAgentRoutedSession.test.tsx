...
async function loadReactHarness({
  ...
  function Probe() {
    ...
    snapshot = useAgent(agentOptions);
    return null;
  }
...
function controlledPublicRunLifecycle(
  runId: string,
    ...
    new ReadableStream({
      start(controller) {
        ...
        preceding.forEach((frame, index) => {
          ...
          controller.enqueue(
            encoder.encode(
              `id: ${runId}:${streamIncarnation}:${index + 2}-0\nevent: ${frame.eventType}\ndata: ${JSON.stringify(event)}\n\n`,
            ),
          );
        });
        finish = () => {
...
function completedPublicRunResponse(
  runId: string,
  ...
  const stream = controlledPublicRunLifecycle(runId, status, preceding);
  stream.finish();
...
test("useAgent invalidates old owners in the auth layout boundary before they can publish", async () => {
  ...
  let authScenarioActive = false;
  const harness = await loadReactHarness({
    onAuthScopeLayout: () => {
      if (authScenarioActive) releaseAtAuthScopeLayout?.();
    },
  });
  await settle(harness.act);
  const { sessionApi } = await import("../../../services/api/session.ts");
  ...
  dom.window.fetch = async () => {
    sseCalls += 1;
    const stream = controlledNonClosingSseHeartbeatResponse();
    closeStreams.push(stream.close);
    return stream.response;
  };
  sessionApi.markRead = async () => {};
  sessionApi.getAuthoritative = async () => ({
    session_id: "session-old-history",
    workspace_id: "workspace-test",
    agent_id: "general-agent",
    title: "Old history",
    purpose: "conversation",
...
test("useAgent keeps generic, network, malformed, legacy, mismatched-ID, and unknown-version GET/retry results fenced", async () => {
  const ambiguousResolvers = [
    ...
    {
      name: "malformed enqueue failure without the exact code",
      resolve: async (submissionId: string) =>
        ({
          submission_id: submissionId,
...
test("useAgent preserves a durable A fence when an option getter publishes then throws", async () => {
  ...
  const originalSubmitChat = sessionApi.submitChat;
  let submitCalls = 0;
  let firedIncarnation = false;
  sessionApi.submitChat = async () => {
    submitCalls += 1;
    throw new Error("the pre-submit ownership check should block this POST");
  };
  const harness = await loadReactHarness({
    agentOptions: {
      getAgentOptions: () => {
  ...
  try {
    await harness.act(async () => {
      assert.deepEqual(await harness.hook.sendMessage("re-entrant auth event"), {
        status: "failed",
      });
    });
    assert.equal(firedIncarnation, true);
    ...
    assert.match(
      ...
      "the resolver key is retained for the original A owner",
    );

    await harness.act(async () => {
      assert.deepEqual(await harness.hook.sendMessage("still fail closed"), {
        status: "failed",
      });
    });
    assert.equal(submitCalls, 0);
...
test("useAgent fences send, resolver, and retry through deferred same-principal refresh", async () => {
  ...
  sessionApi.retryChatSubmissionAdmission = async () => {
    retryCalls += 1;
    throw new Error("refresh fence should block retry admission");
  };
  sessionApi.submitChat = async () => {
    submitCalls += 1;
    throw new Error("refresh fence should block chat admission");
  };
  ...
  try {
    ...
    const resolverCallsBeforeRefreshSettles = resolverCalls.length;
    await harness.act(async () => {
      assert.deepEqual(await harness.hook.sendMessage("must wait for refresh"), {
        status: "failed",
      });
      await harness.hook.retryPendingSubmission();
    });
    ...
    assert.equal(resolverCalls.length, resolverCallsBeforeRefreshSettles);
    ...
    await settle(harness.act);
    assert.deepEqual(
      resolverCalls.slice(resolverCallsBeforeRefreshSettles),
      [submissionA],
    );
    assert.match(
      persistedSubmissionStorageValues().join("\n"),
      new RegExp(submissionA),
      "the settled same-principal refresh must reinstall A's durable reference",
    );
    assert.equal(submitCalls, 0);
    assert.equal(retryCalls, 0);
  } finally {
...
test("useAgent clears only exact enqueue failures from GET and retry resolution", async () => {
  const exactEnqueueFailure = (submissionId: string) =>
    ({
      submission_id: submissionId,
      state: "enqueue_failed",
      submission_disposition: null,
      rejection_code: "queue_enqueue_failed",
      outcome: null,
    }) as unknown as ChatSubmissionResolution;

  for (const resolutionPath of ["GET", "retry"] as const) {
    const harness = await loadReactHarness();
    const { sessionApi } = await import("../../../services/api/session.ts");
    const originalSubmitChat = sessionApi.submitChat;
    const originalGetChatSubmission = sessionApi.getChatSubmission;
    ...
    sessionApi.submitChat = (async () => {
      submissions += 1;
      if (submissions === 1) {
        throw new Error("response lost after enqueue failure");
      }
      return { status: "needs_confirmation", suggestions: [] };
    }) as typeof sessionApi.submitChat;
    sessionApi.getChatSubmission = async (submissionId) => {
      if (resolutionPath === "GET") return exactEnqueueFailure(submissionId);
      throw new ApiRequestError("not found", 404, "chat_submission_not_found");
    };
    sessionApi.retryChatSubmissionAdmission = async (submissionId) => {
      retryAdmissions += 1;
      return exactEnqueueFailure(submissionId);
    };

    try {
      await harness.act(async () => {
        await harness.hook.sendMessage(`enqueue failure via ${resolutionPath}`);
      });
      await settle(harness.act);
      if (resolutionPath === "retry") {
        assert.equal(harness.hook.canRetryPendingSubmission, true);
        await harness.act(async () => {
          await harness.hook.retryPendingSubmission();
        });
      }
      assert.equal(harness.hook.canRetryPendingSubmission, false, resolutionPath);
      assert.ok(harness.hook.error, "the terminal enqueue failure is visible");
      await harness.act(async () => {
        assert.deepEqual(await harness.hook.sendMessage("new submission is allowed"), {
          status: "accepted",
        });
      });
      assert.equal(submissions, 2, resolutionPath);
      assert.equal(retryAdmissions, resolutionPath === "retry" ? 1 : 0);
    } finally {
      sessionApi.submitChat = originalSubmitChat;
      sessionApi.getChatSubmission = originalGetChatSubmission;
      sessionApi.retryChatSubmissionAdmission = originalRetryAdmission;
...
test("useAgent finalizes a failed run once with a Chinese product failure card", async () => {
  ...
  dom.window.fetch = async () => {
    sseCalls += 1;
    return completedPublicRunResponse("run-failed", "failed");
  };
  sessionApi.markRead = async () => {};
  sessionApi.generateTitle = async () => ({
    title: "失败会话",
...
test("useAgent consumes lambchat's runless error then done fallback exactly once", async () => {
  ...
  dom.window.fetch = async () => {
    sseCalls += 1;
    return completedPublicRunResponse("run-fallback-failed", "failed");
  };
  sessionApi.markRead = async () => {};
  sessionApi.generateTitle = async () => ({
    title: "失败回退会话",
  ...
  sessionApi.getEvents = async () => ({
    events: [
      {
        id: "run-fallback-failed:partial",
        event_type: "message:chunk",
  ...
  sessionApi.getEvents = (async (_sessionId, options) => ({
    events: options?.run_id
      ? [{
          ...
          id: "run-transport-interruption:partial",
          event_type: "message:chunk",
          run_id: "run-transport-interruption",
          timestamp: "2026-07-15T00:00:01Z",
          data: {
...
test("useAgent shares one generation-bound reconciliation owner across concurrent recovery callers", async () => {
  ...
  sessionApi.getEvents = (async (_sessionId, options) => ({
    events: options?.run_id
      ? [{
          id: "run-reconcile-owner:partial",
          event_type: "message:chunk",
          run_id: "run-reconcile-owner",
          timestamp: "2026-07-15T00:00:01Z",
          data: { content: "并发恢复中的部分结果" },
        }]
      : [],
  })) as typeof sessionApi.getEvents;
...
test("useAgent releases the active state for succeeded and cancelled terminals", async () => {
  for (const terminal of [
    ...
    const originalFetch = dom.window.fetch;
    dom.window.fetch = async () =>
      completedPublicRunResponse(
        terminal.runId,
        terminal.eventType === "run_cancelled" ? "cancelled" : "succeeded",
      );
    sessionApi.markRead = async () => {};
    sessionApi.generateTitle = async () => ({
      title: "终态会话",
    ...
    sessionApi.getEvents = async () => ({
      events: terminal.cancelled
        ? [
            {
              id: `${terminal.runId}:user`,
              event_type: "user:message",
              run_id: terminal.runId,
              timestamp: "2026-08-21T00:00:00Z",
              data: { run_id: terminal.runId, content: "执行终态任务" },
            },
            {
              id: `${terminal.runId}:cancel`,
              event_type: "user:cancel",
              run_id: terminal.runId,
              timestamp: "2026-08-21T00:00:01Z",
              data: { run_id: terminal.runId },
            },
          ]
        : [
            {
              id: `${terminal.runId}:answer`,
              event_type: "message:chunk",
              run_id: terminal.runId,
              timestamp: "2026-08-21T00:00:01Z",
              data: {
...
test("useAgent uses the backend current run subject before normalizing a failed reload card", async () => {
  ...
  sessionApi.getEvents = async (_sessionId, options) => ({
    ...
    events: options?.run_id
      ? [{
          id: "run-history-failed:partial",
          event_type: "message:chunk",
          run_id: "run-history-failed",
          timestamp: "2026-07-15T00:00:02Z",
          data: { content: "失败前的部分结果" },
        }]
      : [
          {
            id: "evt-history-user",
...
test("useAgent reconnects only the backend-selected active run after reload", async () => {
  ...
  dom.window.fetch = async (input) => {
    ...
    return completedPublicRunResponse("run-history-active", "succeeded");
  };
...
test("useAgent reconciles a reload SSE interruption to its failed run status", async () => {
  ...
  sessionApi.getEvents = async (_sessionId, options) => ({
    ...
    events: options?.run_id
      ? [{
          id: "run-history-interrupted:partial",
          event_type: "message:chunk",
...
test("useAgent rehydrates durable partial text and one fixed failure card", async () => {
  const harness = await loadReactHarness();
  const { sessionApi } = await import("../../../services/api/session.ts");
  const originalGet = sessionApi.get;
  const originalGetEvents = sessionApi.getEvents;
  ...
  const historyEvents = [
    ...
    },
    {
      ...
      data: {
        ...
        event_id: "durable-partial:delta",
        sequence: 1,
        run_id: "run-durable-partial",
        content: "已完成可公开的部分。",
      },
  ...
  sessionApi.getEvents = (async () => {
    ...
    return {
      ...
      events: eventQueries === 1 ? historyEvents.slice(0, 2) : historyEvents,
    };
  }) as typeof sessionApi.getEvents;
  sessionApi.getStatus = (async () => ({
  ...
  try {
    ...
    const runStatusParts = harness.hook.messages
      .flatMap((message) => message.parts || [])
      .filter((part) => part.type === "run_status");
    assert.deepEqual(runStatusParts, [
      {
        type: "run_status",
        event_id: "terminal-failure:run-durable-partial",
