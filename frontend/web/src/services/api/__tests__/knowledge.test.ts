...
test("Knowledge source ACL replacement sends every governed identity set", async () => {
  const originalFetch = globalThis.fetch;
  const calls: Array<{ url: string; body: Record<string, unknown> }> = [];
  globalThis.fetch = async (input, init) => {
    calls.push({
  ...
  try {
    const result = await knowledgeApi.replaceSourceAcl("ks_finance", {
    ...
    assert.match(calls[0]!.url, /\/sources\/ks_finance\/acl$/);
    assert.deepEqual(calls[0]!.body.department_ids, ["财务部"]);
    assert.deepEqual(calls[0]!.body.roles, ["finance_reviewer"]);
    assert.deepEqual(calls[0]!.body.user_ids, ["employee-a"]);
    assert.equal(typeof calls[0]!.body.operation_id, "string");
    assert.deepEqual(result.allowed_roles, ["finance_reviewer"]);
    assert.deepEqual(result.allowed_user_ids, ["employee-a"]);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
