...
import { useSafeAttachmentObjectUrl } from "../../../common/attachmentImageSafety";
...
export function FileRevealItem({
  ...
  const parsed = useMemo(
    () => parseFileRevealPreviewData({ args, result }),
    [args, result],
  );
  ...
  const mediaObjectUrl = useSafeAttachmentObjectUrl(
    parsed.s3Url,
    parsed.mimeType ||
      (isImage ? "image/png" : isVideo ? "video/mp4" : "audio/mpeg"),
    !!parsed.s3Url && canPreview,
  );
      ...
      {canPreview && mediaObjectUrl && success ? (
        ...
        >
          {isAudio ? (
            ...
            <div
              ...
              onClick={() => {
                if (isImage) openImagePreview(parsed.s3Url, mediaObjectUrl);
                else if (isVideo) setVideoViewerSrc(mediaObjectUrl);
              }}
            >
              {!mediaLoaded && (
        ...
        <button
          onClick={() => {
            ...
            if (isImage && parsed.s3Url && mediaObjectUrl) {
              openImagePreview(parsed.s3Url, mediaObjectUrl);
            } else {
              openPreview("manual");
            }
          }}
