...
import { openPreviewUrl } from "../../documents/documentPreviewSources";
import { setActiveRevealPreviewState } from "./items/activeRevealPreviewStore";
import { buildFileLinkPreviewRequest } from "./items/fileLinkPreview";
...
function isPlatformApiMarkdownLink(value: string): boolean {
  ...
  if (!currentOrigin || !hasAbsoluteScheme(trimmed)) {
    return false;
  }
...
function isUnsafeMarkdownLinkUrl(href: string): boolean {
  const trimmed = href.trim();
  if (!trimmed || isAllowedAuthenticatedArtifactFileUrl(trimmed)) {
    return false;
  }

  return isSensitiveInternalPath(trimmed) || isPlatformApiMarkdownLink(trimmed);
}
...
function SafeMarkdownImage({
  ...
  src?: string;
  alt?: string;
  onOpen: (safeSrc: string, imageSrc: string, alt?: string) => void;
}) {
  const safeSrc = resolveSafeSessionImageSrc(src);
  const imageSrc = useSafeAttachmentImageSrc(safeSrc, "image/png");
  if (!safeSrc || !imageSrc) return null;
  ...
  return (
    <img
      ...
      onClick={() => onOpen(safeSrc, imageSrc, alt)}
    />
  );
}
    ...
    <span className="markdown-preview block my-1">
      <ReactMarkdown
        ...
        components={{
          ...
          a: ({ href, children }) => {
            if (href) {
              if (isAllowedAuthenticatedArtifactFileUrl(href)) {
                return (
                  <button
                    type="button"
                    className="align-baseline p-0 bg-transparent border-0 text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 hover:underline transition-colors cursor-pointer"
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      void openPreviewUrl({ url: href });
                    }}
                  >
                    {children}
                  </button>
              ...
              if (isUnsafeMarkdownLinkUrl(href)) {
                return (
          ...
          img: ({ src, alt }) => (
            <SafeMarkdownImage
              src={src}
              alt={alt}
              onOpen={(safeSrc, imageSrc, altText) => {
                sessionImageGallery?.openImage(safeSrc, altText || undefined);
