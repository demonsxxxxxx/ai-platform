...
function parseFileRevealPart(part: ToolPart): ParsedFileReveal | null {
  ...
  if (!parsed) {
    const filePath = sanitizeProjectPath(part.args.path);
    return filePath ? { filePath } : null;
  }

  if (typeof parsed.key === "string" && typeof parsed.url === "string") {
    if (
      isSensitiveInternalPath(parsed.key) ||
      !isAllowedRevealArtifactUrl(parsed.url)
    ) {
      return null;
    }
    ...
    const filePath = sanitizeProjectPath(rawFilePath);
  ...
  if (parsed.type === "file_reveal" && typeof parsed.file === "object") {
    ...
    if (typeof file.s3_key === "string" && isSensitiveInternalPath(file.s3_key)) {
      return null;
    }
    if (
      typeof file.s3_url === "string" &&
      !isAllowedRevealArtifactUrl(file.s3_url)
    ) {
      return null;
    }
