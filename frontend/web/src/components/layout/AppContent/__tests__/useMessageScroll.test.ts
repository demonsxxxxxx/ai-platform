...
test("finds reveal_file tool blocks by sanitized file id targets", () => {
  const messages = [
    {
      parts: [
        {
          type: "tool" as const,
          name: "reveal_file",
          args: { path: "/workspace/.claude/runs/run-1/private.txt" },
          result: {
            id: "file-old",
            key: "revealed/.claude/skills/private.txt",
            name: "private.txt",
            _meta: { path: "/workspace/.claude/runs/run-1/private.txt" },
          },
        },
      ],
    },
