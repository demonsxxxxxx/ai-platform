...
import { isUnsafeExternalHttpDocumentUrl } from "../documents/documentFetchCache";
import {
...
export function resolveSafeRevealedFilePreviewUrl(
  url: string | null | undefined,
): string | null {
  const trimmed = url?.trim();
  if (!trimmed || isSensitiveInternalPath(trimmed)) return null;
  if (isUnsafeExternalHttpDocumentUrl(trimmed)) return null;
...
function normalizeStoredPreview(
  ...
  return {
    ...
    imageUrl: resolveSafeRevealedFilePreviewUrl(stored.image_url) || undefined,
  };
}
