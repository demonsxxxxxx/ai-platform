...
- Git-native release preparation must fail closed when the commit is not
  reachable from fetched main, the versioned checkout is dirty or mismatched,
  an interrupted staging directory remains, a release path escapes through a
  symlink or traversal, or canonical and compatibility image provenance labels
  disagree. A controller-owned post-merge 211 rollout is still required before
  claiming `211 verified` or source-authority closure.
