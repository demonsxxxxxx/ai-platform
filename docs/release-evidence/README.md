...
Foundation Alpha POC runtime subject. The later `5d3d7e2` PR #40 refresh records
the current Foundation Runtime concurrency correctness evidence after the
tightened verifier fixes. Together these remove the narrower
...
subjects, while preserving the boundary that the `5d3d7e2` concurrency run does
