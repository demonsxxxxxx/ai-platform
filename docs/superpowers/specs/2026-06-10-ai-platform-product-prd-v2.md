...
| Foundation Alpha readiness | `tools/foundation_alpha_readiness.py --format json` is the operator-facing S1 summary. It can report `controlled_poc_loop_verified_for_current_source=true` while still blocking production claims, capacity default increases, Docker sandbox hardening, and ordinary-user multi-agent exposure. If `current_source_exact_runtime_commit_match=false`, the summary must also show `runtime_current_for_runtime_relevant_source` with empty runtime-affecting changes before operators treat the controlled POC loop as current. |
