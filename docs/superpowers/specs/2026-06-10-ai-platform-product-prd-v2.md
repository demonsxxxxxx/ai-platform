...
| Foundation Alpha readiness | `tools/foundation_alpha_readiness.py --format json` is the operator-facing S1 summary. It can report `controlled_poc_loop_verified=true` while still blocking production claims, capacity default increases, Docker sandbox hardening, and ordinary-user multi-agent exposure. |
