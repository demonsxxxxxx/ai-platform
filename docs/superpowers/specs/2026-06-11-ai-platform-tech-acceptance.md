...
| Field | Standard |
| --- | --- |
| Current state | Sandbox lease, provider abstraction, callback normalization, fake provider, Docker-provider hardening evidence for reviewed prior runtime subjects, reviewed explicit verifier-path hardening evidence for `9c669761`, reviewed 2026-07-03 live-default G7/FRC evidence for `9c669761`, and reviewed 2026-07-03 label-clean live-default G7/FRC evidence for historical `15903fd` exist; default local provider remains fake. The `9c669761` same-subject evidence pair can support `candidate_evidence_requires_review`, but its status-review artifact records `status_upgrade_decision=not_approved_for_closure`. The historical `15903fd` label-clean evidence pair also records `status=candidate_evidence_requires_review`, `g7_runtime_blocking_reasons=[]`, and `status_upgrade_decision=not_approved_for_closure`. The current post-PR #321 `945db2b` runtime subject now has source-marker reconciliation across repo marker/source snapshot/image labels and all three API/worker in-container marker files. Same-subject Foundation Runtime concurrency evidence, the latest reviewed live-env G7 evidence, approved G7 status-upgrade artifact, and capacity visibility evidence are now `945db2b` entries; the G7 runtime slice is `status_upgrade_approved`, while capacity visibility remains at `blocked_missing_load_test_evidence`. B3 recorded load/profile evidence remains the blocker before any B3 closure, production default increase, or overall gate-closable claim. |
