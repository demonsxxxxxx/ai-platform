...
- [x] Phase 8: PR #309 refreshed to ready-for-review after post-review fixes.
  Evidence: pushed HEAD `98282cd90a39008295da23538bc964bf997810be`; `gh pr ready 309` marked the PR ready for review; REST PR metadata reported `draft=false`, `mergeable=true`, `mergeable_state=clean`; `gh pr checks 309` reported `projection audit, lint, build, trace` pass and `packaged image build` pass; REST review/comment reads returned no formal reviews and no PR comments.
