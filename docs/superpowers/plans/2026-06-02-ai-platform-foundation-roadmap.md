...
- 最新 `main` / 211 边界：本轮 post-PR #321 legacy source-marker cleanup
  ...
  reviewed G7 live-env verifier 和 B3 capacity visibility 已前进到
  `945db2b`：`g7-live-env-hardening-945db2b-live-default-20260704185430`
  已记录 all eight checks passing，B3 capacity visibility 能观测全部 Admin
  Runtime sections，并读到 `blocked_missing_load_test_evidence`。同一 runtime
  subject 的 Foundation Runtime concurrency evidence 现在已记录
  `verified_foundation_runtime_concurrency`，approved G7 status-upgrade
  artifact 也已记录 `approved_for_g7_status_upgrade`。后续 dirty-validation
  `39aa862` strict 10x4 SDK Agent/subagent fanout 和 recorded-batch run 已把
  `b3_10x4_sdk_subagents` profile packet、七门 recorded gate value files、以及
  `capacity-recorded-gate-batch-snapshot-from-live-run.json` 跑到
  `recorded_gate_batch_input_accepted` /
  `readiness.status=ready_for_operator_review`。clean branch-runtime `53887e2`
  recorded batch/profile 复现后，repo-local reviewed release evidence 已进入
  `operator_review_required`，并仍等待 PR/review/merge/operator decision。总体
  仍是 `local partial`：
  不能叫 G7 closure、
...
- Historical progress retained below current state：PR #297 已合入 GitHub `main`
  ...
  `/v1/tasks/execute` HTTP `500`，根因是 executor 在 `cap_drop=["ALL"]`
  下运行却写入 host-user-owned workspace，无法创建 `/workspace/runtime`。
  patched-source diagnostic run
  ...
  source/runtime/OCI labels 和 legacy source alias labels 已读到 `15903fd`，
  API/worker 运行 `ai-platform:15903fd-g7-b3-label-clean-v2`。2026-07-03
  label-clean live-default verifier
  `g7-live-env-hardening-15903fd-label-clean-sudo-20260703055828` 已包装为
  reviewed G7 live-env evidence
  ...
  label-clean operator status-review artifact
...
- 历史 `755e50e` dirty-runtime subject：
  ...
  diagnostic history。因此 `755e50e` 仍只是历史 `local partial` candidate，blocker 已从
  ...
  GitHub issue #164 只作为历史 B0 latest-main runtime-refresh 证据上下文；
  不得作为当前 G7/B3/#164 closure、gate closable 或 production-ready 证据。
  PR #304 `codex/g7-b3-post-300-followup` 已 merge 到 GitHub `main`
  `a9c78efa812efe96b0366011a0c731cb11eb0099`，merge message 明确保留
  G7、B3、Foundation Alpha 和 current-main `211 verified` 未闭合；#164 只作为历史证据上下文，不能用当前 G7/B3 evidence 重新解释为 closure。
  ...
  evidence subject 仍是 `decf33a`：`decf33a` 已新增
  reviewed G7 live-env hardening evidence
  ...
  `a9c78ef` merge commit 变成 current-main `211 verified`，也不构成当前 G7/B3/#164 closure evidence；post-`decf33a` commits 只是 docs/evidence-index/test
  ...
  当前路线读法已从 PR #321 `945db2b` runtime refresh 继续前进到 clean branch-runtime `53887e2` B3 recorded evidence：211 repo-local
  source marker、source snapshot、API/worker image labels 和三个 API/worker
  in-container marker 均已绑定 `945db2b`。最新 reviewed G7 live-env verifier、
  same-subject Foundation Runtime concurrency evidence、approved G7
  status-upgrade evidence 和 capacity visibility evidence 都已前进到
  `945db2b`；当前 `945db2b` 仍不是 G0/G7 closure。clean branch-runtime
  `53887e2` recorded batch/profile evidence 已进入 `operator_review_required`，
  但当前仍不是 B3 closure、不是 Foundation Alpha complete，也不是 G7/B3
  gate-closable。
...
- `ae6b7e5` G7 formal hardening evidence 已补充为
  ...
  它记录 `g7-live-env-hardening-ae6b7e5-20260702045743` 的 8 个 verifier
   ...
   当时 211 live env 读到 executor image 已是
    ...
    audit 读法同样是 `candidate_evidence_requires_review`；PR #304 已 merge
    到 `a9c78ef`，但 `origin/main` merge commit 已比 `decf33a` 更新，当前
    211 仍读到 `decf33a` runtime subject，所以它也不是 current-main
    `211 verified` 或 #164 closure。external env-file label、当前 runtime-affecting source
...
- reviewed `ae6b7e5` Foundation Runtime concurrency evidence 已补充为
  ...
  ordinary-user 平台级 multi-run orchestration，也不构成当前 G7/B3/#164 closure evidence。
- 仍未完成：external runtime env-file label caveat、当前本地 runtime-affecting
  ...
  `blocked_incomplete_inputs`。七个 recorded load-test gates 已由 clean
  branch-runtime `53887e2` evidence 接受，但仍只到 operator review required。
  ...
  2026-07-02 PR #304 runtime subject `decf33a` 又补了一次 read-only
  ...
  `do_not_raise_without_recorded_load_test_evidence`。这只证明 `decf33a`
  runtime 的 B3 visibility / fail-closed 路径；PR #304 已 merge，但这仍不
  ...
  `a294727` post-PR #319 runtime refresh paragraph 和 gate status snapshot 为准；
...
`g7-current-main-ae6b7e5-20260701172910` G7 sandbox evidence、formal hardening
  ...
  API/worker 已跑到 `15903fd` label-clean image，label-clean live-default G7
  hardening evidence 和 same-subject FRC evidence 均已 reviewed/recorded；
  `15903fd` G7 runtime blockers 已清空，可读作
...
evidence 保留：`g7-live-env-hardening-a294727-source-marker-fix-20260704170251`
记录 a294727 G7 verifier all eight checks passing，且 capacity visibility
读到 `blocked_missing_load_test_evidence`，但当时 legacy marker 仍未清理。
PR #321 已 merge 且 211 API/worker 已跑到
`945db2b` image，并修复 repo marker / source snapshot / image labels / 三个
in-container marker 的当前 source-marker mismatch；当前 `945db2b` 已有
reviewed G7 live-env verifier、same-subject Foundation Runtime concurrency
evidence、approved G7 status-upgrade artifact 和 B3 capacity visibility。clean
branch-runtime `53887e2` recorded batch/profile 复现后已有 reviewed release
evidence，但仍不能升级为 G0/G7/B3 gate closure，因为该 evidence 仍是
operator-review-required 而非 closure。
  `decf33a`、`28676df`、`755e50e`、`61073b1`、`bbe23d5`、`a294727`
  和 `945db2b` capacity runtime visibility 都只是 fail-closed visibility
  evidence，不是 recorded load evidence。external env-file redacted readback 可支持 G7
...
`945db2b` 已有 reviewed G7 live-env verifier、source-marker/runtime-health
readback、same-subject Foundation Runtime concurrency evidence、approved G7
status-upgrade artifact 和 reviewed B3 capacity visibility；当前最新 B3 读法是
clean branch-runtime `53887e2` recorded evidence 的
`ready_for_operator_review`。后续 dirty-validation `39aa862` strict
10x4 SDK Agent/subagent fanout 已产生 accepted `b3_10x4_sdk_subagents` profile
packet，且 dirty recorded-batch run 已把七门 gates 与 profile 一起组装到
`recorded_gate_batch_input_accepted` /
`readiness.status=ready_for_operator_review`。clean branch-runtime `53887e2`
recorded batch/profile 复现已进入 reviewed repo-local release evidence。总体
B3 closure 仍是 `local partial`，因为 PR/review/merge/operator decision 和
production-default decision 仍待完成。该 audit 只是控制/计划工件，不是 G7
closure approval 或 B3 closure evidence。
