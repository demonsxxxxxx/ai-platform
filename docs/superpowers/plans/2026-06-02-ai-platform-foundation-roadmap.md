...
- 最新 `main` / 211 边界：本轮 post-PR #321 legacy source-marker cleanup
  ...
  artifact 也已记录 `approved_for_g7_status_upgrade`。但总体仍是
  `local partial`：B3 recorded load evidence 和 `b3_10x4_sdk_subagents` profile
  evidence 仍缺；
  所以不能叫 G7 closure、
...
不能升级为 G0/G7/B3 gate closure，因为 B3 recorded load/profile evidence 仍缺。
...
`blocked_missing_load_test_evidence`。它仍是 `local partial`，因为 B3 recorded
load evidence 和 `b3_10x4_sdk_subagents` profile evidence 未齐备。总体 B3 closure
