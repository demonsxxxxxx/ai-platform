...
conditions, cleanup policy, and required Admin Runtime capture sections
(`capacity`, `database_pool`, `queue`, `admission`, `backpressure`, `sandbox`,
and `observability`) for the seven load-test gates without creating runs,
sending traffic bursts, mutating runtime state, or raising concurrency defaults.
