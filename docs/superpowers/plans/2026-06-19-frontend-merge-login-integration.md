...
- `python -m pytest tests/test_frontend_official_entry_switch_plan.py -q --basetemp .pytest-tmp\frontend-preflight-green` reported `4 passed`.
...
- `tools/frontend_official_entry_switch_plan.py --run-preflight`, uploaded temporarily to `/tmp` and executed on 211, returned schema `ai-platform.frontend-official-entry-preflight.v1`, status `pass`, zero failed checks, `does_not_execute_switch=true`, old PID `25816` matching the recorded old command, `/home/xinlin.jiang/frontend-pr111-smoke/dist/index.html` present, build provenance matching commit `5e3a747e031e7f1a1ce7c525d19a0ca2d64519ed` with dirty `false`, and `GET http://127.0.0.1:8020/api/ai/health` returning `{"status":"ok"}`.
