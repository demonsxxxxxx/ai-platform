...
| #22 office UX/context continuity | Open. Source-level context-pack readiness, user-visible context provenance API projection tests, and executor prompt-injection tests now exist through `tools/office_context_readiness.py`, context route tests, and the worker adapter tests, but context-pack persistence/versioning, 211 executor acceptance, execution-tier router acceptance, document-centric follow-up state, sandbox cold-start latency split, and frontend context provenance acceptance are not implemented by this migration. |
...
- A 211 packaged frontend runtime smoke attempt for commit `305bc40` used the
  ...
  smoked on 211 or another Docker-capable host.
- A 2026-06-12 211 packaged frontend runtime smoke attempt for commit
  `83a500e` synced the current source archive to
  `/home/xinlin.jiang/ai-platform-phaseb/services/ai-platform`, added the
  frontend image `ai-platform.source-revision` label contract, and verified the
  source with `python3 -m compileall -q app tools scripts`. The Docker daemon
  still used a stale registry proxy and failed before runtime smoke: the
  BuildKit path could not resolve the Dockerfile frontend, and the no-syntax
  probe could not pull `node:22-alpine`. The verifier classified the redacted
...
- #22 document-centric context/workbench UX now has a source-level architecture
  ...
  source-level user-visible context provenance API projection tests and bounded
  executor context-pack prompt injection tests, but context-pack
  persistence/versioning, 211 executor acceptance,
