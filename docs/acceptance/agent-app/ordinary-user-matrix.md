...
| Capability truth | User-visible state progresses only through safe meanings for loaded capability, verified invocation, and verified completion. `actually_invoked` is backed only by validated SDK hook evidence for the staged Skill. Selected, staged, inferred, platform-runner, and executor-native facts never claim invocation. |
| Autonomous Skill | A registered Skill may remain visibly uninvoked while the Expert Run succeeds. Once validated hook evidence shows an invocation attempt, a later failure must not be projected as “not invoked”; completion still requires an exact successful terminal hook. |
