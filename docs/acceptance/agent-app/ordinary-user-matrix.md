...
| Capability truth | User-visible state progresses only through safe meanings for loaded capability, verified invocation, and verified completion. `actually_invoked` and bounded attempt/completed/failed counts are backed only by validated SDK hook evidence for the selected Skill or its authorized dependency closure. Missing lifecycle hooks fail before model dispatch. Selected, staged, inferred, platform-runner, and executor-native facts never claim invocation. |
...
| Native tool sandbox | Every governed SDK Run receives the same platform-owned local tool set, while native commands execute in a networkless sidecar that can read only authorized inputs, context, and staged Skills and can write only delivery output through its bounded workspace view. Skill content cannot widen tool, file, credential, network, MCP, or control-plane authority. |
