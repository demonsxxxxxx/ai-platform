...
follow-up state, sandbox cold-start latency split, and frontend context
...
memory reads. Source-level tests now route lightweight writing to
`sdk_only_writing`, document generation/review/translation skills to
`document_worker`, and explicit sandbox/script/browser work to `heavy_sandbox`
without starting Docker during routing. This still does not persist versioned
context packs, provide 211 executor context-pack acceptance, enable long-term
cross-session memory, start Docker for lightweight office tasks, provide
frontend acceptance, or expand G8/G10 multi-agent exposure.
