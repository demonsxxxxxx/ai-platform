...
at `bd690f72723080beeb820d07679da59d84c7913e`. The direct API port
`http://127.0.0.1:18000/api/ai/health` refused connection, while the frontend
proxy `http://127.0.0.1:18001/api/ai/health` returned `{"status":"ok"}`; that
proxy health is not source/runtime parity for current GitHub `main`, not
reviewed G7 release evidence, and not `211 verified` for PR #296.
...
`bd690f7` or current-main `ae6b7e5` G7 release evidence. Current-main smoke
and Foundation Runtime concurrency evidence have not been recorded for
`ae6b7e5`. A prior
`bd690f7` G7
...
current GitHub `main` through PR #296, but that is still source/test progress
only; it still needs deployment and a 211 formal verifier rerun before it can
