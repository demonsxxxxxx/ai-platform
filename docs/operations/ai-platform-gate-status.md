...
runtime-smoke scope. However, `foundation_runtime_concurrency_evidence` remains
a stage blocker because the latest-main concurrency rerun completed only 8
observed concurrent cases/sessions/runs against the required 10+.
...
| --- | --- | --- | --- |
| G0-G1 Source Authority / Security Baseline | Latest-main S2-0 runtime smoke has fresh 211 source/deploy/source-revision/runtime-subject label evidence for `8e0389e`, company-login audit evidence, focused Auth/RBAC smoke evidence, and cross-user/cross-tenant artifact denial evidence; keep under regression. | PRD v2, tech acceptance matrix, roadmap, guardrails, source-authority tests, repo-local compose context, frontend source migration, redacted deploy templates, 2026-06-16 `8e0389e` POC release evidence, and 2026-06-15 `380de6b` historical baseline evidence. | Foundation Runtime concurrency evidence remains blocked for the latest runtime subject; full issue/PR/review closure path, production auth rollout, and release-path/deployment-layout reconciliation are still required before production closure. |
...
| G5 Run Lifecycle / Worker Runtime V1 | Foundation Alpha POC verified queue/run/worker execution and Admin capacity/backpressure projection; the latest-main Foundation Runtime concurrency rerun for `8e0389e` is blocked at 8 observed cases against the 10+ requirement; not capacity-closed. | Tenant-aware queue lease, worker maintenance, active-run admission, bounded metadata, Admin Runtime capacity/backpressure projection, #20 roadmap closure notes, 2026-06-16 `8e0389e` POC verifier evidence, 2026-06-16 `8e0389e` blocked Foundation Runtime concurrency evidence, and 2026-06-15 `380de6b` historical accepted concurrency evidence. | `foundation_runtime_concurrency_evidence` must be rerun or fixed for latest main, #21 is currently closed in GitHub but recorded seven-gate load evidence, large queue bounded lookup pressure, and multi-tenant load evidence are still missing. Production defaults stay unchanged. |
