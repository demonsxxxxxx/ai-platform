...
Current status must be read as PR #308 merged source/runtime progress plus a 211
runtime-image rollout to `15903fdfe96ffcfba9daa1252741111017dcf832`, layered on
top of earlier reviewed `9c669761`, `decf33a`, `4805031`, `ae6b7e5`, and
`28676df` evidence. It is not gate closure. PR #308 is merged into GitHub `main`
at `15903fdfe96ffcfba9daa1252741111017dcf832`; GitHub checks were successful,
`reviewDecision` remained empty, and the PR head before squash merge was
`cc213a995e7ba9808116095365949c4f14b6ac1c`. A read-only 211 poll observed the
repo-local source marker at `15903fdfe96ffcfba9daa1252741111017dcf832` and API
and worker running `ai-platform:15903fd-g7-b3-main-runtime-only-v1`, with
API/worker canonical source/runtime/OCI labels bound to `15903fd`. Some legacy
underscore alias labels still point to `9c669761`; this is a source-authority
cleanup blocker, not B3 capacity evidence. Direct API health on
...
The newer PR #308 runtime subject `15903fd` now has a reviewed live-default G7
hardening entry at
...
readiness builder to compute `verified_foundation_runtime_concurrency`. However, the reviewed operator
status-review artifact at
...
records `status=blocked`, `g7_runtime_blocking_reasons=["stale_runtime_alias_label_mismatch"]`,
and `status_upgrade_decision=not_approved_for_closure`. There is still no
...
evidence. The newer `15903fd` live-default G7/FRC pair advances the deployed
runtime evidence set but remains blocked for status upgrade by stale legacy
alias labels and still does not provide B3 recorded load evidence.
...
marker. The 2026-07-03 readback confirmed API/worker live defaults now use
`SANDBOX_CONTAINER_PROVIDER=docker`,
`SANDBOX_EXECUTOR_IMAGE=ai-platform:15903fd-g7-b3-main-runtime-only-v1`, and
`SANDBOX_EGRESS_POLICY_ENABLED=true`; this removes the current-subject G7
executor-image drift blocker, but stale legacy alias labels and B3 recorded load
...
`15903fd` live-env G7 evidence plus same-subject Foundation Runtime concurrency
...
treat the reviewed `15903fd` operator status-review artifact as a local-partial
record because it explicitly sets
`status_upgrade_decision=not_approved_for_closure` and still lists
`stale_runtime_alias_label_mismatch`.
...
and `SANDBOX_EGRESS_POLICY_ENABLED=true`, while API/worker canonical labels and
repo-local source marker are `15903fd`. The latest current-live `15903fd` G7
audit is blocked by `stale_runtime_alias_label_mismatch`; it is no longer
blocked by `live_api_sandbox_executor_image_not_current_main_bound` or missing
same-subject Foundation Runtime concurrency evidence. The reviewed `15903fd`
G7, FRC, and operator status-review entries still do not close G7 because the
status-review decision is `not_approved_for_closure` and stale legacy alias
cleanup remains open. The older `ae6b7e5` reviewed evidence has no G7 runtime
...
API/worker canonical runtime image labels are at
...
was `not_approved_for_closure`. PR #308 then deployed
`ai-platform:15903fd-g7-b3-main-runtime-only-v1`; reviewed `15903fd` live-default
G7 and same-subject FRC evidence now exist, but the current status remains
`local partial` because legacy underscore alias labels still point to
`9c669761` and B3 recorded load evidence is still missing.
...
| --- | --- | --- | --- |
| G0-G1 Source Authority / Security Baseline | Reviewed B0 runtime-relevant smoke and Foundation Runtime concurrency evidence are retained for `96f27bb`. The current 2026-07-03 observation has GitHub `main`, the 211 repo-local source marker, and 211 API/worker canonical runtime labels at `15903fdfe96ffcfba9daa1252741111017dcf832`, direct API and frontend proxy health returning `{"status":"ok"}`, and the frontend root returning HTTP `200`. Reviewed `ae6b7e5` Foundation Alpha POC evidence is retained as a same-subject set and selected by readiness as historical reviewed evidence, `4805031` has reviewed same-subject G7 live-env hardening and Foundation Runtime concurrency evidence, PR #304 runtime subject `decf33a` has same-subject G7/FRC evidence, PR #305 `28676df` has reviewed B3 capacity visibility plus G7 diagnostic history, PR #306 `9c669761` has reviewed explicit verifier-path G7 hardening evidence, reviewed live-default G7 evidence, same-subject FRC evidence, and a reviewed G7 operator status-review artifact, and PR #308 `15903fd` now has reviewed live-default G7 evidence plus same-subject FRC evidence. This is still not G0 closure: PR #308 is merged and deployed at `15903fd`, but legacy underscore alias labels still point to `9c669761`, and gate closure still requires B3 recorded load evidence, legacy alias/source-authority cleanup, a future approved G7 status-upgrade decision, external env-file/source-authority review, and production auth rollout evidence where applicable; compose labels still point to an external runtime env file and remain an external env-file label caveat / source-authority caveat, not a secret-bearing artifact. | PRD v2, backend phased PRD, technical acceptance matrix, roadmap, guardrails, source-authority tests, repo-local compose context, frontend source migration, redacted deploy templates, 2026-07-03 `15903fd` source/runtime label readback, reviewed live-default G7 hardening entry `2026-07-03-211-g7-sandbox-live-env-hardening-15903fd.json`, `15903fd` FRC evidence entry `2026-07-03-211-foundation-alpha-poc-15903fd-foundation-runtime-concurrency.json`, reviewed G7 operator status-review entry `2026-07-03-211-g7-operator-status-review-15903fd.json`, 2026-07-02 `9c669761` source/runtime label readback, failed G7 verifier diagnostic, reviewed explicit verifier-path G7 hardening entry `2026-07-02-211-g7-sandbox-runtime-hardening-9c669761.json`, reviewed live-default G7 hardening entry `2026-07-03-211-g7-sandbox-live-env-hardening-9c669761.json`, `9c669761` FRC evidence entry `2026-07-03-211-foundation-alpha-poc-9c669761-foundation-runtime-concurrency.json`, reviewed G7 operator status-review entry `2026-07-03-211-g7-operator-status-review-9c669761.json`, 2026-07-02 `28676df` API/worker runtime-image readback and capacity visibility evidence, 2026-07-02 `decf33a` PR #304 runtime-subject G7 live-env hardening evidence, 2026-07-02 `decf33a` verified Foundation Runtime concurrency evidence, 2026-07-02 `ae6b7e5` runtime POC smoke/Auth/RBAC/governance/release-evidence/alert-trace evidence, 2026-07-01 `ae6b7e5` label-repair evidence, 2026-07-02 `4805031` sanitized runtime observation, 2026-07-02 `4805031` reviewed G7 live-env hardening evidence, 2026-07-02 `4805031` verified Foundation Runtime concurrency evidence, 2026-06-30 and older retained historical evidence, 2026-06-15 `380de6b` historical baseline evidence, and issue #164 evidence. | Record B3 load evidence, clean stale legacy runtime alias labels, obtain a future approved G7 status-upgrade decision, complete external env-file/source-authority review, and complete production auth rollout evidence before any G0 closure claim. |
