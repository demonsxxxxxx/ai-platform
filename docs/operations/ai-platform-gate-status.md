...
post-PR #297 split runtime observation with reviewed `4805031` G7 live-env
hardening evidence plus same-subject Foundation Runtime concurrency evidence,
reviewed `ae6b7e5` POC/FRC evidence retained as historical reviewed inputs, and
latest live sandbox executor-image readback, not as gate closure. Current
GitHub `main` is `ba81a0b18da4d4d30c1a8ce44d4bf03bb051fca8`; PR #297 merged at
`4805031fc3333ccbf38224172e4e85e21c0630bb`, and PR #299 merged at
`ba81a0b18da4d4d30c1a8ce44d4bf03bb051fca8`. A fresh 211 read-only poll splits
the state into four layers: the 211 backend source marker still reads
`ae6b7e52c656fd8296cf039834ce8d8559b01228`; API/worker runtime images now run
`ai-platform:4805031-g7-b3-post-297-label-repair-v2`; current live sandbox
executor image now reads `ai-platform:4805031-g7-b3-post-297-label-repair-v2`;
and the frontend image is `ai-platform-frontend:ba81a0b`. The frontend proxy
`http://127.0.0.1:18001/api/ai/health` returned `{"status":"ok"}`. This is
runtime observation plus reviewed G7/FRC evidence for `4805031`; current live
G7 is no longer blocked by executor-image drift. The current live `4805031` G7
audit reads `status=candidate_evidence_requires_review`,
...
It is not current-source `211 verified` because current GitHub `main` is
`ba81a0b`, the 211 backend source marker still reads `ae6b7e5`, B3 load evidence
...
boundaries are tracked separately. A fresh 211 capacity-runtime snapshot for
the running `4805031` image is still pending: the current deployed Admin Runtime
overview returns HTTP `500` on both the default maintenance-cleanup path and the
no-cleanup path because the default stack does not mount the Docker socket into
the API container and the running image still lets Docker SDK container
enumeration fail the whole overview. Local source now has the default-stack fix:
overview container enumeration degrades to
`list_runtime_containers_status=unavailable`, and
`tools/capacity_runtime_evidence.py --skip-maintenance-cleanup` captures the
no-cleanup route. This is local progress only until merged, deployed, and
recaptured on 211.
...
snapshot, current GitHub `main` is `ba81a0b`, 211 backend source still reads
`ae6b7e5`, API/worker runtime images now run
`ai-platform:4805031-g7-b3-post-297-label-repair-v2`, and the frontend image is
`ai-platform-frontend:ba81a0b`. The named
`g7-runtime-probe-20260701203418` formal verifier evidence remains historical
`d318f9f`, the `ae6b7e5` one-shot G7 verifier artifacts for
...
| G7 Sandbox / Resource Hardening | Named 211 runtime-only formal verifier evidence exists for `d318f9f` / `g7-runtime-probe-20260701203418`, covering platform/docker execution, callbacks, cancel cleanup, resource-limit timeout cleanup, egress default-deny evidence, and security options. The `ae6b7e5` verifier artifacts / `g7-current-main-ae6b7e5-20260701172910` are wrapped as repo-local reviewed G7 sandbox release evidence, covering platform/docker execution, token-auth callbacks, cancel cleanup, platform hardening evidence, and all eight verifier checks passing. Later `ae6b7e5` label-repair evidence for `ai-platform:ae6b7e5-g7-b3-label-repair-v1` clears the stale `bd690f7` alias-label mismatch, reviewed formal hardening evidence for `g7-current-main-label-repair-probe-20260701201919` records resource-limit cleanup, egress default-deny, and security-option checks for the explicit verifier path, and reviewed live-env hardening evidence for `g7-live-env-hardening-ae6b7e5-20260702045743` records API/worker defaults using Docker provider, `ae6b7e5` executor image, and egress policy enabled. POC/Auth/Governance/Release/Alert/FRC evidence for `ae6b7e5` is recorded separately as POC correctness evidence. The reviewed `4805031` live-env hardening entry for `g7-live-env-hardening-4805031-20260702023507` records its captured API/worker defaults using Docker provider, `4805031` executor image, egress policy enabled, and all eight verifier checks passing. Same-subject `4805031` FRC evidence is also recorded. Evidence-only audit for `ae6b7e5` and the captured `4805031` evidence can reach `candidate_evidence_requires_review`, and current-live `4805031` G7 is no longer blocked by executor-image drift because fresh 211 readback shows API/worker images, labels, and `SANDBOX_EXECUTOR_IMAGE` all at `4805031`. The reviewed G7 sandbox/POC/FRC/label-repair/hardening entries do not close G7/B3/G0 or production hardening by themselves. External env-file label and current runtime-affecting source rollout are G0/source-authority and production-hardening boundaries, while B3 load evidence is tracked in the B3 row. | Historical B2 sandbox release-evidence, the `d318f9f` named formal verifier observation recorded in this status snapshot, reviewed `ae6b7e5` G7 release-evidence entry `2026-07-01-211-g7-sandbox-runtime-smoke-ae6b7e5.json`, reviewed `ae6b7e5` formal hardening evidence entry `2026-07-01-211-g7-sandbox-runtime-hardening-ae6b7e5.json`, reviewed `ae6b7e5` live-env hardening evidence entry `2026-07-02-211-g7-sandbox-live-env-hardening-ae6b7e5.json`, reviewed `ae6b7e5` label-repair evidence entry `2026-07-01-211-g7-runtime-identity-label-repair-ae6b7e5.json`, reviewed `ae6b7e5` runtime POC smoke entry `2026-07-02-211-foundation-alpha-poc-ae6b7e5-runtime-poc-smoke.json`, `ae6b7e5` Auth/RBAC/governance/release/alert evidence entries, `ae6b7e5` FRC evidence entry `2026-07-02-211-foundation-alpha-poc-ae6b7e5-foundation-runtime-concurrency.json`, reviewed `4805031` G7 live-env hardening evidence entry `2026-07-02-211-g7-sandbox-live-env-hardening-4805031.json`, `4805031` FRC evidence entry `2026-07-02-211-foundation-alpha-poc-4805031-foundation-runtime-concurrency.json`, current-live 211 readback with `SANDBOX_EXECUTOR_IMAGE=ai-platform:4805031-g7-b3-post-297-label-repair-v2`, sanitized `4805031` runtime observation, prior `bd690f7` resource-limit diagnostic, merged verifier-helper callback default tests, and capacity docs expose sandbox limits and missing hardening warnings. | Complete operator status-upgrade review without broadening ordinary-user high-risk sandbox exposure; keep the G0/source-authority and B3 evidence gates separate, and keep B3 blocked until recorded load evidence exists. |
