...
Current status must be read as `source/runtime drift with followups open`, not
as gate closure. As of this snapshot, current main is
`3071a02945c84370f62a9b36884a0a2df8ea9c45`. Live 211 source-marker observation
has seen that commit, while the running `ai-platform-api` and
`ai-platform-worker` containers use the earlier runtime-only image
`ai-platform:d318f9f-g7-b3-runtime-only-v1`; that image's canonical
`ai-platform.source-revision`, `ai-platform.runtime-subject`, and
`org.opencontainers.image.revision` labels point to
`d318f9f6a68b4c17e221eb32705b3f31d349227a`, while legacy underscore labels still
point to `96f27bb9bc8e415faddada2cec0fbfb6ecdcf92c`. The 211 health endpoint
returned `{"status":"ok"}`. A named G7/B3 runtime-only formal verifier run also
passed on 211 as `g7-runtime-probe-20260701203418` for
`ai-platform:d318f9f-g7-b3-runtime-only-v1`: the formal evidence recorded
`runtime_mode=platform`, `sandbox_provider=docker`, running/completed
callbacks, cancel-stops-container, resource-limit timeout cleanup, egress
default-deny evidence, non-privileged security options, and all eight verifier
checks passing. That is named runtime-subject evidence only. It is not a
reviewed local `docs/release-evidence/b2-sandbox/...` entry, not current-main
exact source/runtime evidence, and not source-label parity because the live
source marker and legacy image labels still diverge. Current-main smoke and
Foundation Runtime concurrency evidence have not been recorded. Separately, the
local readiness command for this dirty source tree reports
`source_tree_runtime_affecting_uncommitted_changes_pending_followups_open`
because the working tree still contains runtime-affecting uncommitted changes
and the committed source-runtime relation evidence is older than the current
source tree. It also reports
...
- B3 profile evidence must not be treated as ordinary-user platform-level
  ...
  defaults or close B3 without operator-reviewed recorded evidence.

Next closure work is operational, not semantic: reconcile current-main source,
runtime image labels, reviewed release-evidence records, and runtime evidence on
211; rerun the relevant health, smoke, and Foundation Runtime concurrency
checks; then update this document only with the observed evidence. Before any
status upgrade, `tools/g7_b3_completion_audit.py` can summarize the current
...
| G7 Sandbox / Resource Hardening | Named 211 runtime-only formal verifier evidence exists for `d318f9f` / `g7-runtime-probe-20260701203418`, covering platform/docker execution, callbacks, cancel cleanup, resource-limit timeout cleanup, egress default-deny evidence, and security options. This remains blocked for current-main and high-risk expansion because it is not reviewed local release-evidence, not exact `3071a029` source/runtime proof, and the live API service still defaults to fake sandbox provider posture. | Historical B2 sandbox release-evidence, the `d318f9f` named formal verifier observation recorded in this status snapshot, fake provider remains local/test-only, and capacity docs expose sandbox limits and missing hardening warnings. | Reconcile current-main source/runtime labels, wrap reviewed redacted evidence only when source/image binding is accurate, then rerun Docker provider hardening, egress/quota policy, orphan cleanup, container security options, and Docker-capable 211 smoke without broadening ordinary-user high-risk sandbox exposure. |
