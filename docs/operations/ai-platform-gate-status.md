...
Current status must be read as PR #305 merged document/evidence-state progress
plus a 211 runtime-image rollout to `28676df`, layered on top of earlier
reviewed `decf33a`, `4805031`, and `ae6b7e5` evidence. It is not gate closure.
PR #305 is merged into GitHub `main` at
`28676df4abcbb7063211fceb4cc1701648c43d49`; GitHub checks were successful and
`reviewDecision` remained empty. A read-only 211 poll observed API and worker
running `ai-platform:28676df-g7-b3-current-main-runtime-only-v1`, with
API/worker revision labels bound to `28676df`, and both
`http://127.0.0.1:8020/api/ai/health` and
`http://127.0.0.1:18001/api/ai/health` returning HTTP `200`.
The 211 repo-local source marker still reads
`decf33a017e0b97e2a2992f80e3ccdc19152c1f4`, so G0/source-authority remains
open. The reviewed `28676df` capacity visibility entry records Admin Runtime
HTTP `200`, but readiness remains `blocked_missing_admin_runtime_sections`
because `sandbox` is treated as missing; all seven B3 recorded load gates and
the `b3_10x4_sdk_subagents` profile evidence remain missing. The same-session
G7 hardening verifier path for `28676df` first failed on host Python missing
`pydantic`; after a dependency-safe verifier venv was used, the platform probe
reached the executor but failed with HTTP `500` because the executor process ran
with `cap_drop=["ALL"]` against a host-user-owned workspace and could not create
`/workspace/runtime`. A patched-source diagnostic run
`g7-current-main-28676df-workspace-user-fix-20260702135351` launched the
executor as the workspace owner and passed all eight verifier checks, but that
was not a deployed/reviewed `28676df` runtime-image evidence entry. There is no
reviewed passing `28676df` G7 hardening entry yet. Do not call G7 complete, B3
complete, Foundation Alpha complete, production-ready, `211 verified`, or
`gate closable`.

The reviewed 211 runtime evidence subject immediately before this rollout was
`decf33a017e0b97e2a2992f80e3ccdc19152c1f4`: that subject has reviewed
repo-local G7 live-env hardening evidence plus same-subject Foundation Runtime
concurrency evidence. That evidence-only audit reads
...
It remains historical same-subject evidence, not a current-source closure claim
for `28676df`.
...
than closing it. The later `28676df` rollout changes the running API/worker
image identity but not the G0 source-marker caveat, not the missing passing G7
hardening entry for `28676df`, and not B3 recorded load evidence.
...
Next closure work is operational, not semantic: reuse the reviewed `4805031`
G7 live-env hardening evidence with latest matching live readback plus
same-subject Foundation Runtime concurrency evidence as the G7 runtime input.
Keep the reviewed `ae6b7e5` G7 sandbox, hardening, label-repair,
runtime POC, and FRC evidence as historical reviewed inputs, reconcile the
external env-file label caveat, roll out or otherwise close the current local
runtime-affecting source delta, and complete operator status-upgrade review
without broadening ordinary-user high-risk sandbox exposure.
...
and `SANDBOX_EGRESS_POLICY_ENABLED=true`, while API/worker images and labels
are `4805031`. The latest current-live `4805031` G7 audit is
`candidate_evidence_requires_review` with `blocking_reasons=[]`; it is no
longer blocked by `live_api_sandbox_executor_image_not_current_main_bound`.
The reviewed `4805031` G7 and FRC entries still do not close G7 without
operator status-upgrade review. The older `ae6b7e5` reviewed evidence has no G7 runtime
...
snapshot, current GitHub `main` and the 211 API/worker runtime image labels are
at `28676df4abcbb7063211fceb4cc1701648c43d49`, while the 211 repo-local source
marker still reads `decf33a017e0b97e2a2992f80e3ccdc19152c1f4`. API and frontend
health return HTTP `200`. The named `g7-runtime-probe-20260701203418` formal
verifier evidence remains historical `d318f9f`, the `ae6b7e5` one-shot G7
...
evidence. The current `28676df` capacity visibility entry is reviewed and
redacted, but it remains `blocked_missing_admin_runtime_sections`. The `28676df`
G7 verifier path still lacks reviewed passing deployed-runtime evidence: it
first failed because host Python lacked `pydantic`, then exposed the executor
workspace ownership bug under `cap_drop=["ALL"]`; a patched-source diagnostic
run passed all eight checks but is not a status-upgrade artifact.
...
| G7 Sandbox / Resource Hardening | Named 211 runtime-only formal verifier evidence exists for `d318f9f` / `g7-runtime-probe-20260701203418`, covering platform/docker execution, callbacks, cancel cleanup, resource-limit timeout cleanup, egress default-deny evidence, and security options. Reviewed same-subject G7/FRC evidence exists for `ae6b7e5`, `4805031`, and PR #304 runtime subject `decf33a`; those evidence-only audits can reach `candidate_evidence_requires_review` with `blocking_reasons=[]` for their own runtime subjects. Current 211 API/worker images now run `ai-platform:28676df-g7-b3-current-main-runtime-only-v1` with OCI labels bound to `28676df`, but no reviewed passing `28676df` G7 hardening entry exists yet: the `28676df` verifier path first stopped at `No module named 'pydantic'`, then exposed an executor workspace ownership bug under `cap_drop=["ALL"]` where `/workspace/runtime` could not be created. Patched-source diagnostic run `g7-current-main-28676df-workspace-user-fix-20260702135351` passed all eight verifier checks by running the executor as the workspace owner, but it is not deployed/reviewed runtime-image evidence. The reviewed G7 sandbox/POC/FRC/label-repair/hardening entries do not close G7/B3/G0 or production hardening by themselves. The 211 repo-local source marker still reads `decf33a`, and B3 load evidence is tracked in the B3 row. | Historical B2 sandbox release-evidence, the `d318f9f` named formal verifier observation recorded in this status snapshot, reviewed `ae6b7e5` G7 release-evidence entry `2026-07-01-211-g7-sandbox-runtime-smoke-ae6b7e5.json`, reviewed `ae6b7e5` formal hardening evidence entry `2026-07-01-211-g7-sandbox-runtime-hardening-ae6b7e5.json`, reviewed `ae6b7e5` live-env hardening evidence entry `2026-07-02-211-g7-sandbox-live-env-hardening-ae6b7e5.json`, reviewed `ae6b7e5` label-repair evidence entry `2026-07-01-211-g7-runtime-identity-label-repair-ae6b7e5.json`, reviewed `ae6b7e5` runtime POC smoke entry `2026-07-02-211-foundation-alpha-poc-ae6b7e5-runtime-poc-smoke.json`, `ae6b7e5` Auth/RBAC/governance/release/alert evidence entries, `ae6b7e5` FRC evidence entry `2026-07-02-211-foundation-alpha-poc-ae6b7e5-foundation-runtime-concurrency.json`, reviewed `4805031` G7 live-env hardening evidence entry `2026-07-02-211-g7-sandbox-live-env-hardening-4805031.json`, `4805031` FRC evidence entry `2026-07-02-211-foundation-alpha-poc-4805031-foundation-runtime-concurrency.json`, reviewed `decf33a` PR #304 runtime-subject G7 live-env hardening evidence entry `2026-07-02-211-g7-sandbox-live-env-hardening-decf33a.json`, `decf33a` FRC evidence entry `2026-07-02-211-foundation-alpha-poc-decf33a-foundation-runtime-concurrency.json`, 2026-07-02 `28676df` runtime-image readback and failed G7 verifier diagnostics, patched-source `28676df` workspace-owner diagnostic verifier pass, sanitized `4805031` runtime observation, prior `bd690f7` resource-limit diagnostic, merged verifier-helper callback default tests, and capacity docs expose sandbox limits and missing hardening warnings. | Merge and deploy the workspace-owner executor fix, rerun a dependency-safe `28676df` G7 hardening verifier on the deployed runtime image, complete operator status-upgrade review without broadening ordinary-user high-risk sandbox exposure, keep the G0/source-authority and B3 evidence gates separate, and keep B3 blocked until recorded load evidence exists. |
