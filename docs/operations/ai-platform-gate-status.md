...
Current status must be read as `source/runtime parity with followups open`, not
as gate closure. As of this snapshot, current main is PR #293 merge commit
`bd690f72723080beeb820d07679da59d84c7913e`. Live 211 source-marker observation
shows the target backend source `.ai-platform-source-revision` at the same
commit, and the adjacent source snapshot records
`runtime_affecting_changes_since_runtime_subject=[]`,
`runtime_affecting_dirty_paths=[]`, and `source_tree_dirty=false` for that
synced source tree. The running `ai-platform-api` and `ai-platform-worker`
containers now use runtime-only image
`ai-platform:bd690f7-g7-b3-audit-runtime-only-v1`; their
`ai-platform.source-revision`, `ai-platform.runtime-subject`, and
`org.opencontainers.image.revision` labels all point to
`bd690f72723080beeb820d07679da59d84c7913e`. The 211 health endpoint returned
`{"status":"ok"}` for that `bd690f7` runtime, and a read-only poll found no
recent API/worker error keywords.
...
`bd690f7` G7 release evidence. Current-main smoke and Foundation Runtime
concurrency evidence have not been recorded for `bd690f7`. A current-image G7
...
callback exception path, leaving required callback evidence absent. Local
follow-up now fixes the 211 sandbox evidence generator defaults so Docker
platform probes bind the callback receiver on `0.0.0.0` and publish
`http://host.docker.internal:{port}/callback` unless the operator explicitly
overrides callback settings. That is source/test progress only; it still must
be merged, deployed, and rerun on 211 before it can become reviewed G7 release
evidence. The last clean-source readiness command reported
`source_synced_runtime_pending_followups_open`; the current worktree contains
pending verifier-helper and status-doc changes, so treat this turn as
`local partial`, not clean-source gate closure.
...
- B3 profile evidence must not be treated as ordinary-user platform-level
  ...
  bounded B3 probe was explicitly `probe_completed_not_gate_evidence`; it
  observed only a small bounded `api_read_write_burst` probe and the evidence
  bundle stayed `blocked_incomplete_load_test_evidence`, with the seven
  recorded load-test gates still missing.

Next closure work is operational, not semantic: merge/deploy the local G7
...
Foundation Runtime concurrency checks for the selected runtime subject.
...
runtime or load evidence by itself. The running API container still has
`SANDBOX_CONTAINER_PROVIDER=fake` and
`SANDBOX_EXECUTOR_IMAGE=ai-platform:local`, so this snapshot also must not be
used as a production Docker sandbox hardening claim. Until those checks and
evidence close, do not call the current source `211 verified`, `gate closable`,
B3 complete, G7 complete, Foundation Alpha complete, or production-ready.
...
| G7 Sandbox / Resource Hardening | Named 211 runtime-only formal verifier evidence exists for `d318f9f` / `g7-runtime-probe-20260701203418`, covering platform/docker execution, callbacks, cancel cleanup, resource-limit timeout cleanup, egress default-deny evidence, and security options. The current `bd690f7` runtime has source/runtime label parity and health, and a current-image probe reached Docker/resource-limit evidence, but G7 remains blocked because that attempt left no callback exception evidence under no-masq/default-deny egress, the live API service still defaults to fake sandbox provider posture, and there is no reviewed local `bd690f7` G7 release-evidence entry. Local source now includes a verifier-helper default fix for Docker platform callback reachability, but that is not reviewed 211 G7 evidence until merged, deployed, and rerun. | Historical B2 sandbox release-evidence, the `d318f9f` named formal verifier observation recorded in this status snapshot, current `bd690f7` source/runtime parity observation, current-image resource-limit diagnostic, local verifier-helper callback default tests, fake provider remains local/test-only, and capacity docs expose sandbox limits and missing hardening warnings. | Produce reviewed redacted G7 evidence for the selected current runtime subject with Docker provider hardening, callback exception evidence under no-masq/default-deny egress, quota policy, orphan cleanup, container security options, and Docker-capable 211 smoke without broadening ordinary-user high-risk sandbox exposure. |
| G8 Platform Multi-Run Controlled Beta | Deferred parking-lot for platform-owned multi-run orchestration. The legacy title was "G8 Multi-Agent Controlled Beta", but it must not be read as ordinary-user platform-level multi-run product exposure. SDK-internal agent/subagent behavior stays inside one governed platform run, and the current question is B3 deployment capacity for SDK subagent fanout rather than a G8 product route. This is a blocked expansion, not a current Foundation Alpha `open_followups` item. | Historical dispatcher and child-run admission work exists behind controls but is not the current product route; Claude Agent SDK remains the execution-layer route for agent/subagent patterns. | Do not build a separate agent harness as the next step. Reopen only after prior gates have recorded tenant quota/backpressure, model-gateway pressure, sandbox pressure, artifact/event volume, token/cost accounting, rollback, and no ordinary-user platform-level multi-run orchestration exposure evidence. |
